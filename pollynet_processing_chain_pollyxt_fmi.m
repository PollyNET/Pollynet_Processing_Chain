...
flagSaturation = pollyxt_fmi_saturationdetect(data, config);
...
[data, overlapAttri] = pollyxt_fmi_overlap(data, config);
fprintf('[%s] Finish.\n', tNow());

%% split the cloud free profiles into continuous subgroups
fprintf('\n[%s] Start to split the cloud free profiles.\n', tNow());
cloudFreeGroups = pollyxt_fmi_splitcloudfree(data, config);
...
[temperature, pressure, relh, meteorAttri] = pollyxt_fmi_readmeteor(data, config);
...
[data.refHIndx355, data.refHIndx532, data.refHIndx1064, data.dpIndx355, data.dpIndx532, data.dpIndx1064] = pollyxt_fmi_rayleighfit(data, config);
...
[data.IWV, IWVAttri] = pollyxt_fmi_read_IWV(data, config);
...
[data.wvconstUsed, data.wvconstUsedStd, data.wvconstUsedInfo] = pollyxt_fmi_select_wvconst(wvconst, wvconstStd, data.IWVAttri, polly_parsetime(taskInfo.dataFilename, config.dataFileFormat), fullfile(processInfo.results_folder, campaignInfo.name, config.wvCaliFile), config.flagUsePreviousLC);
...
data.LC = LC;
LCUsed = struct();
[LCUsed.LCUsed355, LCUsed.LCUsedTag355, LCUsed.flagLCWarning355, LCUsed.LCUsed532, LCUsed.LCUsedTag532, LCUsed.flagLCWarning532, LCUsed.LCUsed1064, LCUsed.LCUsedTag1064, LCUsed.flagLCWarning1064, LCUsed.LCUsed387, LCUsed.LCUsedTag387, LCUsed.flagLCWarning387, LCUsed.LCUsed607, LCUsed.LCUsedTag607, LCUsed.flagLCWarning607] = pollyxt_fmi_mean_LC(data, config, taskInfo, fullfile(processInfo.results_folder, config.pollyVersion));
data.LCUsed = LCUsed;
fprintf('[%s] Finish.\n', tNow());

%% attenuated backscatter
fprintf('\n[%s] Start to calculate attenuated backscatter.\n', tNow());
[att_beta_355, att_beta_532, att_beta_1064, att_beta_387, att_beta_607] = pollyxt_fmi_att_beta(data, config);
...
tc_mask_V2 = pollyxt_fmi_targetclassi_V2(data, config);
...
if processInfo.flagEnableCaliResultsOutput
    ...
    pollyxt_fmi_save_wvconst(wvconst, wvconstStd, wvCaliInfo, data.IWVAttri, taskInfo.dataFilename, data.wvconstUsed, data.wvconstUsedStd, fullfile(processInfo.results_folder, campaignInfo.name, config.wvCaliFile));
...
if processInfo.flagEnableDataVisualization
    ...
    pollyxt_fmi_display_quasiretrieving_V2(data, taskInfo, config);
    ...
    pollyxt_fmi_display_targetclassi_V2(data, taskInfo, config);
    ...
    pollyxt_fmi_display_longterm_cali(taskInfo, config);
