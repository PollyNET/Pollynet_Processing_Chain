    function [] = pollyxt_dwd_display_WV(data, taskInfo, config)
%pollyxt_dwd_display_WV display the water vapor mixing ratio and relative humidity.
%   Example:
%       [] = pollyxt_dwd_display_WV(data, taskInfo, config)
%   Inputs:
...
elseif strcmpi(processInfo.visualizationMode, 'python')
    ...
    %% display rcs 
    save(fullfile(tmpFolder, 'tmp.mat'), 'figDPI', 'WVMR', 'RH', 'lowSNRMask', 'flagCalibrated', 'height', 'time', 'processInfo', 'campaignInfo', 'taskInfo', 'xtick', 'xtickstr', '-v7');
    tmpFile = fullfile(tmpFolder, 'tmp.mat');
    flag = system(sprintf('%s %s %s %s', fullfile(processInfo.pyBinDir, 'python'), fullfile(pyFolder, 'pollyxt_dwd_display_WV.py'), tmpFile, saveFolder));
    if flag ~= 0
        warning('Error in executing %s', 'pollyxt_dwd_display_WV.py');
    end
