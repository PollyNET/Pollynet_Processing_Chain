function [] = pollyxt_dwd_display_longterm_cali(taskInfo, config)
%pollyxt_dwd_display_longterm_cali Display the lidar constants.
%   Example:
...
LC = pollyxt_dwd_read_LC(lcCaliFile, config.dataFileFormat);
...
[depolCaliTime532, depolCaliConst532] = pollyxt_dwd_read_depolconst(depolCaliFile532);
...
elseif strcmpi(processInfo.visualizationMode, 'python')
    ...
    if flag ~= 0
        warning('Error in executing %s', 'pollyxt_dwd_display_longterm_cali.py');
    end
    delete(tmpFile);
