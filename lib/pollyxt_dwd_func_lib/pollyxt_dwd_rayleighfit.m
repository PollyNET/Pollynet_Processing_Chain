function [refH355, refH532, refH1064, dpIndx355, dpIndx532, dpIndx1064] = pollyxt_dwd_rayleighfit(data, config)
%pollyxt_dwd_rayleighfit Find the reference height with rayleigh fitting algorithm.
%   Example:
%       [refH355, refH532, refH1064] = pollyxt_dwd_rayleighfit(data, config)
%   Inputs:
%		data: struct
...
for iGroup = 1:size(data.cloudFreeGroups, 1)
    ...
    SNR532 = polly_SNR(sig532PC, bg532PC);
    [molBsc532, molExt532] = rayleigh_scattering(532, pressure, temperature + 273.17, 380, 70);
    molSig532 = molBsc532 .* exp(-2 * cumsum(molExt532 .* [data.distance(1), diff(data.distance)]));

    [thisRefH532, thisDpIndx532] = polly_rayleighfit(data.distance, sig532PC, sig532PCR, bg532PC, molSig532, config.minDecomLogDist532, config.heightFullOverlap(config.isFR & config.isTot & config.is532nm), config.maxDecomHeight532, config.maxDecomThickness532, config.decomSmoothWin532, config.minRefThickness532, config.minRefDeltaExt532, config.minRefSNR532);
    ...
    SNR1064 = polly_SNR(sig1064PC, bg1064PC);
    [molBsc1064, molExt1064] = rayleigh_scattering(1064, pressure, temperature + 273.17, 380, 70);
    molSig1064 = molBsc1064 .* exp(-2 * cumsum(molExt1064 .* [data.distance(1), diff(data.distance)]));

    [thisRefH1064, thisDpIndx1064] = polly_rayleighfit(data.distance, sig1064PC, sig1064PCR, bg1064PC, molSig1064, config.minDecomLogDist1064, config.heightFullOverlap(config.isFR & config.isTot & config.is1064nm), config.maxDecomHeight1064, config.maxDecomThickness1064, config.decomSmoothWin1064, config.minRefThickness1064, config.minRefDeltaExt1064, config.minRefSNR1064);

    % concatenate the results
    refH355 = cat(1, refH355, thisRefH355);
    refH532 = cat(1, refH532, thisRefH532);
    refH1064 = cat(1, refH1064, thisRefH1064);
    dpIndx355{end + 1} = dpIndx355;
    dpIndx532{end + 1} = dpIndx532;
    dpIndx1064{end + 1} = dpIndx1064;
end
