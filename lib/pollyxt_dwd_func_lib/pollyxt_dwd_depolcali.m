function [data, depCalAttri] = pollyxt_dwd_depolcali(data, config, taskInfo)
...
if sum(~ isnan(depol_cal_fac_532)) < 1 && config.flagUsePreviousDepolCali
    ...
    [depol_cal_fac_532, depol_cal_fac_std_532, depol_cal_time_532] = pollyxt_dwd_search_history_depolconst(mean(time), fullfile(processInfo.results_folder, taskInfo.pollyVersion, config.depolCaliFile532), datenum(0,1,7,0,0,0), defaults, 532);
