...
def polly_1v2_display_monitor(tmpFile, saveFolder):
    '''
    Description
    -----------
    ...
    Usage
    -----
    polly_1v2_display_monitor(tmpFile)
    ...
    try:
        ...
        time = mat['monitorStatus']['time'][0][0][0]
        mTime = mat['mTime'][0][:]
    ...
    ExtPyro = np.concatenate(ExtPyro)
    Temp1064 = np.concatenate(Temp1064)
    Temp1 = np.concatenate(Temp1)
    Temp2 = np.concatenate(Temp2)
    OutsideT = np.concatenate(OutsideT)
    OutsideRH = np.concatenate(OutsideRH)
    roof = np.concatenate(roof)
    rain = np.concatenate(rain)
    shutter = np.concatenate(shutter)

    # filter out the invalid values
    ExtPyro = np.ma.masked_where(np.isnan(ExtPyro), ExtPyro)
    Temp1064 = np.ma.masked_where(np.isnan(Temp1064), Temp1064)
    ...
    flags = np.ma.vstack((np.transpose(rain), np.transpose(roof), np.transpose(shutter)))
    ...
    pcmesh = ax4.pcolormesh(time, np.arange(flags.shape[0] + 1), flags, cmap=cmap, vmin=-0.5, vmax=4.5)
...
def main():
    polly_1v2_display_monitor('C:\\Users\\zhenping\\Desktop\\Picasso\\tmp\\tmp.mat', 'C:\\Users\\zhenping\\Desktop\\Picasso\\recent_plots\\POLLY_1V2\\20181031')

if __name__ == '__main__':
    # main()
    polly_1v2_display_monitor(sys.argv[1], sys.argv[2])