...
def pollyDisplayVDR355(tmpFile, saveFolder):
    """
    Description
    -----------
    ...
    Usage
    -----
    pollyDisplayVDR355(tmpFile, saveFolder)
    ...
    depCalMask = np.tile(depCalMask, (vdr355.shape[0], 1))
    fogMask = np.tile(fogMask, (vdr355.shape[0], 1))
    ...
    vdr355 = np.ma.masked_where(depCalMask != 0, vdr355)
    vdr355 = np.ma.masked_where(fogMask == 1, vdr355)
...
if __name__ == '__main__':
    # main()
    pollyDisplayVDR355(sys.argv[1], sys.argv[2])
