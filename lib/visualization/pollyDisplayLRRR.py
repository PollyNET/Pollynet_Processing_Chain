...
def pollyDisplayLRRR(tmpFile, saveFolder):
    """
    Description
    -----------
    ...
    Usage
    -----
    pollyDisplayLRRR(tmpFile, saveFolder)
    ...
    try:
        ...
        height = mat['height'][:][0]
        time = mat['time'][:][0]
        LR355_RR = mat['LR355_RR'][:][0]
        LR532_RR = mat['LR532_RR'][:][0]
        LR1064_RR = mat['LR1064_RR'][:][0]
    ...
    # display LR with RR method
    fig = plt.figure(figsize=[5, 8])
    ax = fig.add_axes([0.21, 0.15, 0.74, 0.75])
    p1, = ax.plot(LR355_RR, height, color='#0000ff',
                  linestyle='-', label='355 nm', zorder=2)
    p2, = ax.plot(LR532_RR, height, color='#00b300',
                  linestyle='-', label='532 nm', zorder=2)
    p3, = ax.plot(LR532_RR, height, color='#e60000',
...
if __name__ == '__main__':
    # main()
    pollyDisplayLRRR(sys.argv[1], sys.argv[2])
