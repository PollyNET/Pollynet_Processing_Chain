...
def pollyDisplaySigStatus387FR(tmpFile, saveFolder):
    """
    Description
    -----------
    ...
    Usage
    -----
    pollyDisplaySigStatus387FR(tmpFile)
    ...
    # display status of 387 FR
    fig = plt.figure(figsize=[10, 5])
    ax = fig.add_axes([0.11, 0.15, 0.74, 0.75])
    pcmesh = ax.pcolormesh(
        Time, Height, SAT_FR_387,
    ...
    fig.savefig(os.path.join(
        saveFolder, '{dataFilename}_SAT_FR_387.{imgFmt}'.format(
...
if __name__ == '__main__':
    # main()
    pollyDisplaySigStatus387FR(sys.argv[1], sys.argv[2])
