...
if strcmpi(PicassoConfig.visualizationMode, 'matlab')
    ...
    picFile = fullfile(PicassoConfig.pic_folder, CampaignConfig.name, datestr(data.mTime(1), 'yyyy'), datestr(data.mTime(1), 'mm'), datestr(data.mTime(1), 'dd'), sprintf('%s_monitor.png', rmext(PollyDataInfo.dataFilename)));
...
elseif strcmpi(PicassoConfig.visualizationMode, 'python')
    ...
    flag = system(sprintf('%s %s %s %s', fullfile(PicassoConfig.pyBinDir, 'python'), fullfile(pyFolder, 'pollyxt_dwd_displayHousekeeping.py'), tmpFile, saveFolder));
    if flag ~= 0
        warning('Error in executing %s', 'pollyxt_dwd_displayHousekeeping.py');
    end
    delete(tmpFile);
