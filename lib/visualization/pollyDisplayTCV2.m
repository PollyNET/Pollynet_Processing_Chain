function pollyDisplayTCV2(data)
...
if (sum(flag532T) == 1) && (sum(flag532C) == 1) && (sum(flag1064) == 1) && (sum(flag607) == 1)
    ...
    imgFormat = PollyConfig.imgFormat;
        ...
        flag = system(sprintf('%s %s %s %s', fullfile(PicassoConfig.pyBinDir, 'python'), fullfile(pyFolder, 'pollyDisplayTCV2.py'), tmpFile, saveFolder));
        if flag ~= 0
