...
def pollyDisplay_cloudinfo(nc_dict, config_dict, polly_conf_dict, saveFolder):
    """
    Description
    -----------
    ...
    ## read from nc file
    # dimensions are: time, layer_index
    # variables are: layer_index, cloud_base_height, cloud_top_height, cloud_phase, cloud_phase_probability
    cloud_base_height = nc_dict['cloud_base_height']
    quality_mask = np.where(cloud_base_height > 0, 0, 0)
...
#    height = nc_dict['height']
    ...
    dataFilename = re.split(r'_cloudinfo',nc_dict['PollyDataFile'])[0]
    ...
    cloud_base_height, quality_mask = readout.fill_time_gaps_of_matrix(time, cloud_base_height, quality_mask)
