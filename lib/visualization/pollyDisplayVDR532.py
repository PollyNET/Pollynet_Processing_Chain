...
def pollyDisplayVDR532(tmpFile, saveFolder):
    """
    Description
    -----------
    ...
    Usage
    -----
    pollyDisplayVDR532(tmpFile, saveFolder)
    ...
    depCalMask = np.tile(depCalMask, (vdr532.shape[0], 1))
    fogMask = np.tile(fogMask, (vdr532.shape[0], 1))
    ...
    vdr532 = np.ma.masked_where(depCalMask != 0, vdr532)
    vdr532 = np.ma.masked_where(fogMask == 1, vdr532)
...
if __name__ == '__main__':
    # main()
    pollyDisplayVDR532(sys.argv[1], sys.argv[2])
