...
if strcmpi(PicassoConfig.visualizationMode, 'matlab')
    ...
    picFile = fullfile(PicassoConfig.pic_folder, CampaignConfig.name, datestr(data.mTime(1), 'yyyy'), datestr(data.mTime(1), 'mm'), datestr(data.mTime(1), 'dd'), sprintf('%s_monitor.%s', rmext(PollyDataInfo.dataFilename), imgFormat));
    ...
    title(sprintf('Housekeeping data from %s at %s', PollyDataInfo.pollyVersion, CampaignConfig.location), 'Interpreter', 'none', 'FontWeight', 'bold', 'FontSize', 8);
    ...
    text(0.90, -0.5, sprintf('Version %s', PicassoConfig.programVersion), 'Units', 'Normal', 'FontSize', 8);
    ...
    set(findall(gcf, '-Property', 'FontName'), 'FontName', PicassoConfig.fontname);
    export_fig(gcf, picFile, sprintf('-r%d', PicassoConfig.figDPI), '-transparent');
    close();
...
elseif strcmpi(PicassoConfig.visualizationMode, 'python')
    ...
    flag = system(sprintf('%s %s %s %s', fullfile(PicassoConfig.pyBinDir, 'python'), fullfile(pyFolder, 'polly_1v2_displayHousekeeping.py'), tmpFile, saveFolder));
    if flag ~= 0
        warning('Error in executing %s', 'polly_1v2_displayHousekeeping.py');
    end
    delete(tmpFile);
