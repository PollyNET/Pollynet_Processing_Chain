function [] = pollyxt_fmi_display_longterm_cali(taskInfo, config)
%pollyxt_fmi_display_longterm_cali Display the lidar constants.
%   Example:
...
LC = pollyxt_fmi_read_LC(lcCaliFile, config.dataFileFormat);
...
[WVCaliTime, WVCaliFlag, WVConst] = pollyxt_fmi_read_wvconst(wvCaliFile);
...
[depolCaliTime355, depolCaliConst355] = pollyxt_fmi_read_depolconst(depolCaliFile355);
...
[depolCaliTime532, depolCaliConst532] = pollyxt_fmi_read_depolconst(depolCaliFile532);
...
elseif strcmpi(processInfo.visualizationMode, 'python')
    ...
    flag = system(sprintf('%s %s %s %s', fullfile(processInfo.pyBinDir, 'python'), fullfile(pyFolder, 'pollyxt_fmi_display_longterm_cali.py'), tmpFile, saveFolder));
    if flag ~= 0
        warning('Error in executing %s', 'pollyxt_fmi_display_longterm_cali.py');
    end
    delete(tmpFile);
