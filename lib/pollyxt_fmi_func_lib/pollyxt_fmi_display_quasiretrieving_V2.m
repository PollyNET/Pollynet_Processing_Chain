function [] = pollyxt_fmi_display_quasiretrieving_V2(data, taskInfo, config)
%pollyxt_fmi_display_quasiretrieving_V2 display the quasi retrievings results
%   Example:
...
if strcmpi(processInfo.visualizationMode, 'matlab')
    ...
    file_quasi_bsc_1064 = fullfile(processInfo.pic_folder, campaignInfo.name, datestr(data.mTime(1), 'yyyy'), datestr(data.mTime(1), 'mm'), datestr(data.mTime(1), 'dd'), sprintf('%s_Quasi_Bsc_1064_V2.png', rmext(taskInfo.dataFilename)));
    ...
    file_quasi_AngExp_532_1064 = fullfile(processInfo.pic_folder, campaignInfo.name, datestr(data.mTime(1), 'yyyy'), datestr(data.mTime(1), 'mm'), datestr(data.mTime(1), 'dd'), sprintf('%s_Quasi_ANGEXP_532_1064_V2.png', rmext(taskInfo.dataFilename)));
    ...
    xlabel('UTC');
    ylabel('Height (m)');
    title(sprintf('Quasi Backscatter Coefficient (V2) at %snm for %s at %s', '1064', campaignInfo.name, campaignInfo.location), 'fontweight', 'bold', 'interpreter', 'none');
    set(gca, 'Box', 'on', 'TickDir', 'out');
    set(gca, 'ytick', 0:2000:12000, 'yminortick', 'on');
    [xtick, xtickstr] = timelabellayout(data.mTime, 'HH:MM');
    set(gca, 'xtick', xtick, 'xticklabel', xtickstr);
    ...
    xlabel('UTC');
    ylabel('Height (m)');
    title(sprintf('Quasi BSC Angstroem Exponent 532-1064 (V2) for %s at %s', campaignInfo.name, campaignInfo.location), 'fontweight', 'bold', 'interpreter', 'none');
    set(gca, 'Box', 'on', 'TickDir', 'out');
    set(gca, 'ytick', 0:2000:12000, 'yminortick', 'on');
    [xtick, xtickstr] = timelabellayout(data.mTime, 'HH:MM');
    set(gca, 'xtick', xtick, 'xticklabel', xtickstr);
...
elseif strcmpi(processInfo.visualizationMode, 'python')
    ...
    flag = system(sprintf('%s %s %s %s', fullfile(processInfo.pyBinDir, 'python'), fullfile(pyFolder, 'pollyxt_fmi_display_quasiretrieving_V2.py'), tmpFile, saveFolder));
    if flag ~= 0
        warning('Error in executing %s', 'pollyxt_fmi_display_quasiretrieving_V2.py');
    end
