function [] = pollyxt_fmi_display_targetclassi_V2(data, taskInfo, config)
%pollyxt_fmi_display_targetclassi_V2 display the target classification reuslts V2
%   Example:
...
elseif strcmpi(processInfo.visualizationMode, 'python')
    ...
    flag = system(sprintf('%s %s %s %s', fullfile(processInfo.pyBinDir, 'python'), fullfile(pyFolder, 'pollyxt_fmi_display_targetclassi_V2.py'), tmpFile, saveFolder));
    if flag ~= 0
        warning('Error in executing %s', 'pollyxt_fmi_display_targetclassi_V2.py');
    end
