...
if sum(~ isnan(depol_cal_fac_355)) < 1 && config.flagUsePreviousDepolCali && config.flagDepolCali
    % Apply historical calibration results
    [depol_cal_fac_355, depol_cal_fac_std_355, depol_cal_time_355] = pollyxt_fmi_search_history_depolconst(mean(time), fullfile(processInfo.results_folder, campaignInfo.name, config.depolCaliFile355), datenum(0,1,7,0,0,0), defaults, 355);
