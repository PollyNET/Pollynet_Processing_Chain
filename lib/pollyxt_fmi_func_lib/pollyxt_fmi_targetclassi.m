function [tc_mask] = pollyxt_fmi_targetclassi(data, config)
%pollyxt_fmi_targetclassi Aerosol target classification based on algorithms presented in H.Baar et al, 2017, ATM.
%   Example:
...
flag_small_par_beta_1064 = data.quasi_par_beta_1064 < config.clear_thres_par_beta_1064;
flag_large_par_beta_1064 = (data.quasi_par_beta_1064 >= config.turbid_thres_par_beta_1064);
flag_large_par_beta_532 = (data.quasi_par_beta_532 >= config.turbid_thres_par_beta_532);
flag_water_par_depol = data.quasi_parDepol_532 < config.droplet_thres_par_depol;
flag_small_par_depol = (data.quasi_parDepol_532 < config.spheroid_thres_par_depol);
flag_medium_par_depol = (data.quasi_parDepol_532 < config.unspheroid_thres_par_depol) & (data.quasi_parDepol_532 >= config.spheroid_thres_par_depol);
flag_large_par_depol = (data.quasi_parDepol_532 >= config.unspheroid_thres_par_depol);
flag_ice_par_depol = data.quasi_parDepol_532 >= config.ice_thres_par_depol;
flag_ice_vol_depol = data.volDepol_532 >= config.ice_thres_vol_depol;
flag_large_ang = data.quasi_ang_532_1064 >= config.large_thres_ang;
flag_small_ang = data.quasi_ang_532_1064 <= config.small_thres_ang;
...
% cloud mask
flag_cloud = flag_cloud_search(data.height, data.quasi_par_beta_1064, config);
...
data.quasi_par_beta_532(1:hIndxFullOverlap, :) = NaN;
