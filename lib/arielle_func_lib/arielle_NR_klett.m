function [aerBsc355_NR_klett, aerBsc532_NR_klett, aerExt355_NR_klett, aerExt532_NR_klett, refBeta_NR_355_klett, refBeta_NR_532_klett] = arielle_NR_klett(data, config)
%arielle_NR_klett Retrieve aerosol optical properties from NR channels with klett method
%Example:
%   [aerBsc355_NR_klett, aerBsc532_NR_klett, aerExt355_NR_klett, 
%    aerExt532_NR_klett, refBeta_NR_355_klett, refBeta_NR_532_klett] = 
%       arielle_NR_klett(data, config)
%Inputs:
%   data.struct
...
%       aerosol backscatter coefficient at 355 nm with klett method.
%       [m^{-1}*sr^{-1}] 
%   aerBsc532_NR_klett: matrix
%       aerosol backscatter coefficient at 532 nm with klett method.
%       [m^{-1}*sr^{-1}] 
%   aerExt355_NR_klett: matrix
%       aerosol extinction coefficient at 355 nm with klett method.
%       [m^{-1}]
%   aerExt532_NR_klett: matrix
%       aerosol extinction coefficient at 355 nm with klett method. [m^{-1}] 
%   refBeta_NR_355_klett: array
%       reference value for retrieving the NR aerosol backscatter at 355 nm.
%       [m^{-1}*sr^{-1}]
%   refBeta_NR_532_klett: array
%       reference value for retrieving the NR aerosol backscatter at 532 nm.
%       [m^{-1}*sr^{-1}]
%History:
...
for iGroup = 1:size(data.cloudFreeGroups, 1)
    ...
    if (~ isnan(data.refHIndx355(iGroup, 1))) && (any(flagChannel355_NR))
        ...
        if (~ flagRefSNRLow_355) && (~ isnan(refBeta355))
            [thisAerBsc355_NR_klett, ~] = polly_fernald(data.distance0, ...
                sig355, config.LR_NR_355, refH355, refBeta355, molBsc355, ...
                config.smoothWin_klett_NR_355);
    ...
    refBeta_NR_355_klett = cat(2, refBeta_NR_355_klett, refBeta355);
end
...
for iGroup = 1:size(data.cloudFreeGroups, 1)
    ...
    if (~ isnan(data.refHIndx532(iGroup, 1))) && (any(flagChannel532_NR))
        ...
        if (~ flagRefSNRLow_532) && (~ isnan(refBeta532))
            [thisAerBsc532_NR_klett, ~] = polly_fernald(data.distance0, ...
                sig532, config.LR_NR_532, refH532, refBeta532, molBsc532, ...
                config.smoothWin_klett_NR_532);
    ...
    refBeta_NR_532_klett = cat(2, refBeta_NR_532_klett, refBeta532);
end
