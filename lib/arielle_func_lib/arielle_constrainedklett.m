function [aerBsc355_aeronet, aerBsc532_aeronet, aerBsc1064_aeronet, aerExt355_aeronet, aerExt532_aeronet, aerExt1064_aeronet, LR355_aeronet, LR532_aeronet, LR1064_aeronet, deltaAOD355, deltaAOD532, deltaAOD1064] = arielle_constrainedklett(data, AERONET, config)
%arielle_constrainedklett Retrieve the aerosol optical properties with the
%constrains from AERONET.
%Example:
%   [aerBsc355_aeronet, aerBsc532_aeronet, aerBsc1064_aeronet,
%    aerExt355_aeronet, aerExt532_aeronet, aerExt1064_aeronet,
%    LR355_aeronet, LR532_aeronet,
%    LR1064_aeronet] = arielle_constrainedklett(data, AERONET, config)
%Inputs:
%   data.struct
...
for iGroup = 1:size(data.cloudFreeGroups, 1)
    ...
    if ~ isnan(data.refHIndx1064(iGroup, 1))
        ...
        [molBsc1064, ~] = rayleigh_scattering(1064, data.pressure(iGroup, :), data.temperature(iGroup, :) + 273.17, 380, 70);
