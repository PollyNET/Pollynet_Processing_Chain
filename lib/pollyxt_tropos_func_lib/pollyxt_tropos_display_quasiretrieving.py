...
def pollyxt_tropos_display_quasiretrieving(tmpFile, saveFolder):
    ...
    ax.set_title('Quasi backscatter coefficient at {wave}nm from {instrument} at {location}'.format(wave=355, instrument=pollyVersion, location=location), fontsize=15)
    ...
    fig.savefig(os.path.join(saveFolder, '{dataFilename}_Quasi_Bsc_355.png'.format(dataFilename=rmext(dataFilename))), dpi=figDPI)
    plt.close()
...
def main():
    pollyxttroposs_display_quasiretrieving('C:\\Users\\zhenping\\Desktop\\Picasso\\tmp\\tmp.mat', 'C:\\Users\\zhenping\\Desktop')
