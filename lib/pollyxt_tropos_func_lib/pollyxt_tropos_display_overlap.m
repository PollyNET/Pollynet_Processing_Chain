...
%   Example:
%       [] = pollyxt_tropos_display_overlap(data, taskInfo, attri, config)
%   Inputs:
%       data: struct
%       taskInfo: struct
...
elseif strcmpi(processInfo.visualizationMode, 'python')
    ...
    flag = system(sprintf('%s %s %s %s', fullfile(processInfo.pyBinDir, 'python'), fullfile(pyFolder, 'pollyxt_tropos_display_overlap.py'), tmpFile, saveFolder));
    if flag ~= 0
        warning('Error in executing %s', 'pollyxt_tropos_display_overlap.py');
    end
