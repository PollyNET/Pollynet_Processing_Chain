function [data, depCalAttri] = pollyxt_tropos_depolcali(data, config, taskInfo)
%pollyxt_tropos_depolcali calibrate the polly depol channels both for 355 and 532 nm with +- 45\deg method.
%	Example:
%		[data] = pollyxt_tropos_depolcali(data, config)
%	Inputs:
%		data: struct
...
if sum(~ isnan(depol_cal_fac_532)) < 1 && config.flagUsePreviousDepolCali
    ...
    [depol_cal_fac_532, depol_cal_fac_std_532, depol_cal_time_532] = pollyxt_tropos_search_history_depolconst(mean(time), fullfile(processInfo.results_folder, taskInfo.pollyVersion, config.depolCaliFile532), datenum(0,1,7,0,0,0), defaults, 532);
...
if sum(~ isnan(depol_cal_fac_355)) < 1 && config.flagUsePreviousDepolCali
    % Apply historical calibration results
    [depol_cal_fac_355, depol_cal_fac_std_355, depol_cal_time_355] = pollyxt_tropos_search_history_depolconst(mean(time), fullfile(processInfo.results_folder, taskInfo.pollyVersion, config.depolCaliFile355), datenum(0,1,7,0,0,0), defaults, 355);
