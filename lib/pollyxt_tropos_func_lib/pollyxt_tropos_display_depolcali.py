...
def pollyxt_tropos_display_depolcali(tmpFile, saveFolder):
    '''
    Description
    -----------
    ...
    Usage
    -----
    pollyxt_tropos_display_depolcali(tmpFile)
    ...
    ax1.set_xlabel('Signal (a.u.)', fontweight='semibold', fontsize=15)
    ...
    fig.text(0.5, 0.98, 'Depolarization Calibration for {wave}nm at {start}-{end}'.format(wave=wavelength, start=start.strftime('%H:%M'), end=end.strftime('%H:%M')), horizontalalignment='center', fontweight='bold', fontsize=15)
    ...
    ax2.set_xlim([0, 50])
    ax2.set_xlabel('Ratio', fontweight='semibold', fontsize=15)
...
def main():
    pollyxt_tropos_display_depolcali('C:\\Users\\zhenping\\Desktop\\Picasso\\tmp\\tmp.mat', 'C:\\Users\\zhenping\\Desktop')

if __name__ == '__main__':
    # main()
    pollyxt_tropos_display_depolcali(sys.argv[1], sys.argv[2])