...
def pollyxt_tropos_display_depolcali(tmpFile, saveFolder):
    ...
    fig.text(0.5, 0.98, 'Depolarization Calibration for {wave}nm at {start}-{end}'.format(wave=wavelength, start=start.strftime('%H:%M'), end=end.strftime('%H:%M')), horizontalalignment='center', fontsize=15)
