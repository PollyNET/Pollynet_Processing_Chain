function [aerBsc355_klett, aerBsc532_klett, aerBsc1064_klett, aerExt355_klett, aerExt532_klett, aerExt1064_klett] = pollyxt_tropos_OC_klett(data, config)
%pollyxt_tropos_OC_klett Retrieve aerosol optical properties for overlap correction far-range signal with klett method
%   Example:
%       [aerBsc355_klett, aerBsc532_klett, aerBsc1064_klett, aerExt355_klett, aerExt532_klett, aerExt1064_klett] = pollyxt_tropos_OC_klett(data, config)
%   Inputs:
%       data.struct
...
for iGroup = 1:size(data.cloudFreeGroups, 1)
    ...
    if ~ isnan(data.refHIndx355(iGroup, 1))
        ...
        sig355 = transpose(squeeze(sum(data.signal355OverlapCor(:, data.cloudFreeGroups(iGroup, 1):data.cloudFreeGroups(iGroup, 2)), 2)));
        bg355 = transpose(squeeze(sum(data.bg355OverlapCor(:, data.cloudFreeGroups(iGroup, 1):data.cloudFreeGroups(iGroup, 2)), 2)));
...
for iGroup = 1:size(data.cloudFreeGroups, 1)
    ...
    if ~ isnan(data.refHIndx532(iGroup, 1))
        ...
        sig532 = transpose(squeeze(sum(data.signal532OverlapCor(:, data.cloudFreeGroups(iGroup, 1):data.cloudFreeGroups(iGroup, 2)), 2)));
        bg532 = transpose(squeeze(sum(data.bg532OverlapCor(:, data.cloudFreeGroups(iGroup, 1):data.cloudFreeGroups(iGroup, 2)), 2)));
...
for iGroup = 1:size(data.cloudFreeGroups, 1)
    ...
    if ~ isnan(data.refHIndx1064(iGroup, 1))
        ...
        sig1064 = squeeze(sum(data.signal1064OverlapCor(:, data.cloudFreeGroups(iGroup, 1):data.cloudFreeGroups(iGroup, 2)), 2));
        bg1064 = squeeze(sum(data.bg1064OverlapCor(:, data.cloudFreeGroups(iGroup, 1):data.cloudFreeGroups(iGroup, 2)), 2));
