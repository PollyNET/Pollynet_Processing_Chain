...
## check wether operating in wsl or in native linux
os_name=$(uname -s)
# Check if it's Linux
if [[ "$os_name" == "Linux" ]]; then
    # Additional check for WSL
    if grep -qi Microsoft /proc/version; then
        echo "This script is running within Windows Subsystem for Linux (WSL)."
	os_name="Windows"
	# Replace backslashes with forward slashes
        filenamepath_linux_slashes=${PICASSO_TODO_FILE//\\/\/}
	drive_letter="${filenamepath_linux_slashes:0:1}"
	# Replace the leading drive letter with "/mnt/<drive_letter>"
        PICASSO_TODO_FILE="/mnt/$(echo "$drive_letter" | tr '[:upper:]' '[:lower:]')${filenamepath_linux_slashes:2}"
    else
        echo "This is a native Linux operating system."
    fi
else
...
/mnt/c/Programme/MATLAB/R2014b/bin/matlab.exe -nodisplay -nodesktop -nosplash <<ENDMATLAB
cd $PICASSO_DIR;
initPicassoToolbox;
clc;
