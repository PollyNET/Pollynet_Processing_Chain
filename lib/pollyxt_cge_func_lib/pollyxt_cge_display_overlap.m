function [] = pollyxt_cge_display_overlap(data, taskInfo, attri, config)
%pollyxt_cge_display_overlap display the overlap function.
%   Example:
%       [] = pollyxt_cge_display_overlap(data, taskInfo, attri, config)
%   Inputs:
%       data: struct
%       taskInfo: struct
...
elseif strcmpi(processInfo.visualizationMode, 'python')
    ...
    flag = system(sprintf('%s %s %s %s', fullfile(processInfo.pyBinDir, 'python'), fullfile(pyFolder, 'pollyxt_cge_display_overlap.py'), tmpFile, saveFolder));
    if flag ~= 0
        warning('Error in executing %s', 'pollyxt_cge_display_overlap.py');
    end
