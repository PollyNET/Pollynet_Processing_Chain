function pollyxt_cge_display_targetclassi_V2(data, taskInfo, config)
%POLLYXT_CGE_DISPLAY_TARGETCLASSI_V2 display the target classification reuslts
%Example:
...
if strcmpi(processInfo.visualizationMode, 'matlab')
    %% initialization 
    fileTC = fullfile(processInfo.pic_folder, campaignInfo.name, datestr(data.mTime(1), 'yyyy'), datestr(data.mTime(1), 'mm'), datestr(data.mTime(1), 'dd'), sprintf('%s_TC_V2.%s', rmext(taskInfo.dataFilename), imgFormat));
...
elseif strcmpi(processInfo.visualizationMode, 'python')
    ...
    flag = system(sprintf('%s %s %s %s', fullfile(processInfo.pyBinDir, 'python'), fullfile(pyFolder, 'pollyxt_cge_display_targetclassi_V2.py'), tmpFile, saveFolder));
    if flag ~= 0
        warning('Error in executing %s', 'pollyxt_cge_display_targetclassi_V2.py');
    end
    delete(tmpFile);
