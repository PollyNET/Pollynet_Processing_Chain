function [wvconstUsed, wvconstUsedStd, wvconstUsedInfo] = select_wvconst(wvconst, wvconstStd, IWVAttri, currentTime, file, flagUsePreviousWVConst)
%select_wvconst  select the most appropriate water vapor 
%calibration constant to calculate the WVMR and RH.
%   Example:
%       [wvconstUsed, wvconstUsedStd, wvconstUsedInfo] = 
%           select_wvconst(wvconst, wvconstStd, WVCaliInfo, 
...
if isempty(wvconst)
    [preWVCaliConst, preWVCaliConstStd] = load_wvconst(currentTime, file, ...
        pollyType, 'deltaTime', deltaTime, 'flagClosest', true);
    [wvconstUsed, wvconstUsedStd] = arielle_search_wvconst(currentTime, ...
