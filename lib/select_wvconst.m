function [wvconstUsed, wvconstUsedStd, wvconstUsedInfo] = ...
    select_wvconst(wvconst, wvconstStd, IWVAttri, queryTime, ...
                   dbFile, pollyType, varargin)
%select_wvconst  select the most appropriate water vapor calibration constant.
%Example:
%   [wvconstUsed, wvconstUsedStd, wvconstUsedInfo] = 
%       select_wvconst(wvconst, wvconstStd, 
%                      IWVAttri, queryTime, dbFile, pollyType)
%Inputs:
%   wvconst: array
...
if (any(~ isnan(wvconst))) && p.Results.flagUsePrevWVConst
    ...
    [preWVCaliConst, preWVCaliConstStd, ~, ~, caliInstrument] = ...
        load_wvconst(queryTime, dbFile, pollyType, 'deltaTime', ...
