function [depCal_P_Ang_time_start, depCal_P_Ang_time_end, depCal_N_Ang_time_start, depCal_N_Ang_time_end, maskDepCal] = polly_depolCal_time(depCalAng, mTime, init_depAng, maskDepCalAng)
%polly_depolCal_time Retrieve the time for the polly depolarization calibration period.
%                    depolarization calibration: 5 min (+45) + 5 min (-45) + 0.5 min (?)
%   Example:
%       [depCal_P_Ang_time_start, depCal_N_Ang_time_start, maskDepCal] = polly_depolCal_time(depCalAng, mTime, init_depAng, maskDepCalAng)
%   Inputs:
%       depCalAng: array
...
%           e.g., {'none', 'none', 'p', 'p', 'p', 'p', 'p', 'p', 'p', 'p', 'none', 'none', 'n', 'n', 'n', 'n', 'n', 'n', 'n', 'n', 'none'};   % 'p' for positive angle, 'n' for negative angle and 'none' for invalid profile.
%   Outputs:
%       depCal_P_Ang_time_start: scalar
%           time for the first profile with valid positive angle depolarization calibration. [datenum]
%       depCal_P_Ang_time_end: scalar
%           time for the last profile with valid positive angle depolarization calibration. [datenum]
%       depCal_N_Ang_time_start: scalar
%           time for the first profile with valid negative angle depolarization calibration. [datenum]
%       depCal_N_Ang_time_end: scalar
%           time for the last profile with valid negative angle depolarization calibration. [datenum]
...
flagDepCal = ~ (abs(depCalAng - init_depAng) <= 0.5);   % the profile will be treated as depol cali profile if it has different depol_cal_ang than the init_depAng
...
for iDepCalPeriod = 1:nDepCalPeriods
    ...
    if isempty(t_all_n_depCal) | isempty(t_all_p_depCal)
        warning('There are no profiles for p/n depolarization calibration. Please check the data.');
        return;
    end
