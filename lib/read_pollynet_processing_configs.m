...
%   Inputs:
%       file: char
%           pollynet processing configurature files. This file is used to guide the processing chain to search for the suitable config file and processing function to processing the polly data. More detailed information can be found in doc/pollynet.md
%   Outputs:
...
if ~ exist(file, 'file')
    error('Error in read_pollynet_processing_configs: pollynet history configuration file does not exist. \n%s\n', file);
end
...
try
    ...
    for iRow = 1:length(data{1})
        ...
        pollyConfigHistory.startTime = [pollyConfigHistory.startTime, datenum(data{2}{iRow}, 'yyyy-mm-dd HH:MM:SS')];
        pollyConfigHistory.endTime = [pollyConfigHistory.endTime, datenum(data{3}{iRow}, 'yyyy-mm-dd HH:MM:SS')];
