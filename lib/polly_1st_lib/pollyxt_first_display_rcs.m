function [] = pollyxt_first_display_rcs(data, taskInfo, config)
%pollyxt_first_display_rcs display the range corrected signal, vol-depol at 355 and 532 nm
%   Example:
%       [] = pollyxt_first_display_rcs(data, config)
%   Inputs:
%       data.struct
...
elseif strcmpi(processInfo.visualizationMode, 'python')
    ...
    save(tmpFile, 'figDPI', 'mTime', 'height', 'fogMask', 'yLim_FR', 'yLim_NR', 'RCS_FR_532', 'RCS_NR_532', 'processInfo', 'campaignInfo', 'taskInfo', 'xtick', 'xtickstr', 'RCS532FRColorRange', 'RCS532NRColorRange', '-v6');
    flag = system(sprintf('%s %s %s %s', fullfile(processInfo.pyBinDir, 'python'), fullfile(pyFolder, 'pollyxt_first_display_rcs.py'), tmpFile, saveFolder));
    if flag ~= 0
        warning('Error in executing %s', 'pollyxt_first_display_rcs.py');
    end
    delete(tmpFile);
