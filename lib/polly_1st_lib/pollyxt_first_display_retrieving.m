function [] = pollyxt_first_display_retrieving(data, taskInfo, config)
%pollyxt_first_display_retrieving display aerosol optical products
%   Example:
...
elseif strcmpi(processInfo.visualizationMode, 'python')
    ...
    for iGroup = 1:size(data.cloudFreeGroups, 1)
        ...
        save(tmpFile, 'figDPI', 'startIndx', 'endIndx', 'rcs532', 'height', 'time', 'molRCS532', 'refHIndx532', 'aerBsc_532_klett', 'aerBsc_532_raman', 'aerExt_532_klett', 'aerExt_532_raman', 'LR532_raman', 'meteorSource', 'temperature', 'pressure', 'processInfo', 'campaignInfo', 'taskInfo', 'yLim_FR', 'yLim_NR', 'rcsLim', 'aerBscLim', 'aerExtLim', 'aerLRLim', '-v6');
        flag = system(sprintf('%s %s %s %s', fullfile(processInfo.pyBinDir, 'python'), fullfile(pyFolder, 'pollyxt_first_display_retrieving.py'), tmpFile, saveFolder));
        if flag ~= 0
            warning('Error in executing %s', 'pollyxt_first_display_retrieving.py');
        end
        delete(tmpFile);
    end
else
