function [liconst, liconstStd, caliStartTime, caliStopTime] = ...
    load_liconst(queryTime, dbFile, pollyType, ...
        wavelength, caliMethod, varargin)
%load_liconst load lidar calibration constant from database.
%Example:
%   [liconst, liconstStd, caliStartTime, caliStopTime] = 
%       load_liconst(queryTime, dbFile, pollyType, wavelength, caliMethod)
%Inputs:
%   queryTime: datenum
%       query time.
%   dbFile: char
...
parse(p, queryTime, dbFile, pollyType, wavelength, caliMethod, varargin{:});
    ...
    try
        data = fetch(conn, ...
            [sprintf(['SELECT lc.cali_start_time, lc.cali_stop_time, ', ...
                ...
                'AND (lc.cali_method = ''%s'') '], pollyType, wavelength, caliMethod), ...
    ...
    try
        data = fetch(conn, ...
            [sprintf(['SELECT lc.cali_start_time, lc.cali_stop_time, ', ...
                    ...
                    '(lc.cali_method = ''%s'') '], pollyType, wavelength, caliMethod), ...
            condWithinDT, condBeforeQuery, ...
