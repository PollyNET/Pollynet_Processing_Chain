function [] = pollyxt_tjk_display_retrieving(data, taskInfo, config)
%pollyxt_tjk_display_retrieving display aerosol optical products
%   Example:
...
elseif strcmpi(processInfo.visualizationMode, 'python')
    ...
    for iGroup = 1:size(data.cloudFreeGroups, 1)
        ...
        flag = system(sprintf('%s %s %s %s', fullfile(processInfo.pyBinDir, 'python'), fullfile(pyFolder, 'pollyxt_tjk_display_retrieving.py'), tmpFile, saveFolder));
        if flag ~= 0
            warning('Error in executing %s', 'pollyxt_tjk_display_retrieving.py');
        end
