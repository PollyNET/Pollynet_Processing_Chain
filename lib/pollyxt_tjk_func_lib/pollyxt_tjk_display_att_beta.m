function [] = pollyxt_tjk_display_att_beta(data, taskInfo, config)
%pollyxt_tjk_display_att_beta display attenuated signal
%   Example:
...
elseif strcmpi(processInfo.visualizationMode, 'python')
    ...
    %% display rcs 
    save(fullfile(tmpFolder, 'tmp.mat'), 'figDPI', 'ATT_BETA_355', 'ATT_BETA_532', 'ATT_BETA_1064', 'quality_mask_355', 'quality_mask_532', 'quality_mask_1064', 'height', 'time', 'flagLC355', 'flagLC532', 'flagLC1064', 'att_beta_cRange_355', 'att_beta_cRange_532', 'att_beta_cRange_1064', 'processInfo', 'campaignInfo', 'taskInfo', 'xtick', 'xtickstr', '-v7');
    tmpFile = fullfile(tmpFolder, 'tmp.mat');
    flag = system(sprintf('%s %s %s %s', fullfile(processInfo.pyBinDir, 'python'), fullfile(pyFolder, 'pollyxt_tjk_display_att_beta.py'), tmpFile, saveFolder));
    if flag ~= 0
        warning('Error in executing %s', 'pollyxt_tjk_display_att_beta.py');
    end
