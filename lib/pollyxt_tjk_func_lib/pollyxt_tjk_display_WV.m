    function [] = pollyxt_tjk_display_WV(data, taskInfo, config)
%pollyxt_tjk_display_WV display the water vapor mixing ratio and relative humidity.
%   Example:
...
elseif strcmpi(processInfo.visualizationMode, 'python')
    ...
    %% display rcs 
    save(fullfile(tmpFolder, 'tmp.mat'), 'figDPI', 'WVMR', 'RH', 'lowSNRMask', 'flagCalibrated', 'height', 'time', 'processInfo', 'campaignInfo', 'taskInfo', 'xtick', 'xtickstr', 'WVMRColorRange', '-v7');
    tmpFile = fullfile(tmpFolder, 'tmp.mat');
    flag = system(sprintf('%s %s %s %s', fullfile(processInfo.pyBinDir, 'python'), fullfile(pyFolder, 'pollyxt_tjk_display_WV.py'), tmpFile, saveFolder));
    if flag ~= 0
        warning('Error in executing %s', 'pollyxt_tjk_display_WV.py');
    end
