function [] = pollyxt_tjk_display_overlap(data, taskInfo, attri, config)
%pollyxt_tjk_display_overlap display the overlap function.
%   Example:
%       [] = pollyxt_tjk_display_overlap(height, overlap532, overlap355, overlap532Defaults, overlap355Defaults, file, config, campaignInfo)
%   Inputs:
%       height: array
...
elseif strcmpi(processInfo.visualizationMode, 'python')
    ...
    flag = system(sprintf('%s %s %s %s', fullfile(processInfo.pyBinDir, 'python'), fullfile(pyFolder, 'pollyxt_tjk_display_overlap.py'), tmpFile, saveFolder));
    if flag ~= 0
        warning('Error in executing %s', 'pollyxt_tjk_display_overlap.py');
    end
