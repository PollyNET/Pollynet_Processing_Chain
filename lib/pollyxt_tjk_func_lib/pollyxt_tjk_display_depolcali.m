function [] = pollyxt_tjk_display_depolcali(data, taskInfo, attri)
%pollyxt_tjk_display_depolcali display the depolarization calibration results
%   Example:
%       [] = pollyxt_tjk_display_depolcali(attri)
%   Inputs:
%       attri
%   Outputs:
%       
...
elseif strcmpi(processInfo.visualizationMode, 'python')
	...
	for iCali = 1:length(attri.depCalAttri532.thisCaliTime)  
	    ...
	    flag = system(sprintf('%s %s %s %s', fullfile(processInfo.pyBinDir, 'python'), fullfile(pyFolder, 'pollyxt_tjk_display_depolcali.py'), tmpFile, saveFolder));
	    if flag ~= 0
	        warning('Error in executing %s', 'pollyxt_tjk_display_depolcali.py');
	    end
