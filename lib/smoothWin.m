...
%SMOOTHWIN smooth the signal with a varied window width
%   Example:
...
%           if win is a scalar, the signal will be smoothed by a fixed sliding window;
%           if win is a matrix, the window can be specified by the win matrix in different range
%       method: char
...
if ismatrix(win)
    if size(win, 2) == 3
        for iWin = 1:size(win, 1)
            ...
            signalSM(win(iWin, 1):win(iWin, 2)) = tmp((win(iWin, 1) - startIndx + 1):(win(iWin, 2) - startIndx + 1));
        end
    end
end
