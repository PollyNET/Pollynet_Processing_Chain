function [] = pollyxt_tau_display_saturation(data, taskInfo, config)
%pollyxt_tau_display_saturation display the saturation mask.
%   Example:
...
elseif strcmpi(processInfo.visualizationMode, 'python')
    ...
    flag = system(sprintf('%s %s %s %s', fullfile(processInfo.pyBinDir, 'python'), fullfile(pyFolder, 'pollyxt_tau_display_saturation.py'), tmpFile, saveFolder));
    if flag ~= 0
        warning('Error in executing %s', 'pollyxt_tau_display_saturation.py');
    end
