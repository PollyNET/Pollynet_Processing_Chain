function [voldepol355_klett, pardepol355_klett, pardepolStd355_klett, voldepol355_raman, pardepol355_raman, pardepolStd355_raman, moldepol355, moldepolStd355, flagDefaultMoldepol355, voldepol532_klett, pardepol532_klett, pardepolStd532_klett, voldepol532_raman, pardepol532_raman, pardepolStd532_raman, moldepol532, moldepolStd532, flagDefaultMoldepol532] = pollyxt_tau_OC_depolratio(data, config)
%pollyxt_tau_OC_depolratio retrieve volume depolarization ratio and particle depolarization ratio with overlap corrected signal.
%   Example:
%       [voldepol355_klett, pardepol355_klett, pardepolStd355_klett, voldepol355_raman, pardepol355_raman, pardepolStd355_raman, moldepol355, moldepolStd355, flagDefaultMoldepol355, voldepol532_klett, pardepol532_klett, pardepolStd532_klett, voldepol532_raman, pardepol532_raman, pardepolStd532_raman, moldepol532, moldepolStd532, flagDefaultMoldepol532] = pollyxt_tau_OC_depolratio(data, config)
%   Inputs:
%       data.struct
...
for iGroup = 1:size(data.cloudFreeGroups, 1)
    ...
    if ~ isnan(data.refHIndx355(iGroup, 1))
        ...
        if ~ isnan(data.aerBsc355_klett(iGroup, 80))
            ...
            [thisPardepol355_klett, thisPardepolStd355_klett] = polly_parDepol(thisVoldepol355_klett, thisVoldepoStdl355_klett, data.aerBsc355_OC_klett(iGroup, :), ones(size(data.aerBsc355_OC_klett(iGroup, :))) * 1e-7, molBsc355, thisMoldepol355, thisMoldepolStd355);

            % based with raman retrieved bsc
            if ~ isnan(data.aerBsc355_raman(iGroup, 80))
                [thisPardepol355_raman, thisPardepolStd355_raman] = polly_parDepol(thisVoldepol355_raman, thisVoldepoStdl355_raman, data.aerBsc355_OC_raman(iGroup, :), ones(size(data.aerBsc355_OC_raman(iGroup, :))) * 1e-7, molBsc355, thisMoldepol355, thisMoldepolStd355);
            end
        end
    end
...
for iGroup = 1:size(data.cloudFreeGroups, 1)
    ...
    if ~ isnan(data.refHIndx532(iGroup, 1))
        ...
        if ~ isnan(data.aerBsc532_klett(iGroup, 80))
            ...
            % based with klett retrieved bsc
            [thisPardepol532_klett, thisPardepolStd532_klett] = polly_parDepol(thisVoldepol532_klett, thisVoldepoStdl532_klett, data.aerBsc532_OC_klett(iGroup, :), ones(size(data.aerBsc532_OC_klett(iGroup, :))) * 1e-7, molBsc532, thisMoldepol532, thisMoldepolStd532);

            % based with raman retrieved bsc
            if ~ isnan(data.aerBsc532_raman(iGroup, 80))
                [thisPardepol532_raman, thisPardepolStd532_raman] = polly_parDepol(thisVoldepol532_raman, thisVoldepoStdl532_raman, data.aerBsc532_OC_raman(iGroup, :), ones(size(data.aerBsc532_OC_raman(iGroup, :))) * 1e-7, molBsc532, thisMoldepol532, thisMoldepolStd532);
            end
        end
    end
