function [] = pollyxt_tau_display_rcs(data, taskInfo, config)
%pollyxt_tau_display_rcs display the range corrected signal, vol-depol at 355 and 532 nm
%   Example:
%       [] = pollyxt_tau_display_rcs(data, config)
%   Inputs:
%		data: struct
...
elseif strcmpi(processInfo.visualizationMode, 'python')
    ...
    flag = system(sprintf('%s %s %s %s', fullfile(processInfo.pyBinDir, 'python'), fullfile(pyFolder, 'pollyxt_tau_display_rcs.py'), tmpFile, saveFolder));
    if flag ~= 0
        warning('Error in executing %s', 'pollyxt_tau_display_rcs.py');
    end
