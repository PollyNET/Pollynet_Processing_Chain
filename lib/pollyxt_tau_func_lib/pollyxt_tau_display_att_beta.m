function [] = pollyxt_tau_display_att_beta(data, taskInfo, config)
%pollyxt_tau_display_att_beta display attenuated signal
%   Example:
...
elseif strcmpi(processInfo.visualizationMode, 'python')
    ...
    flag = system(sprintf('%s %s %s %s', fullfile(processInfo.pyBinDir, 'python'), fullfile(pyFolder, 'pollyxt_tau_display_att_beta.py'), tmpFile, saveFolder));
    if flag ~= 0
        warning('Error in executing %s', 'pollyxt_tau_display_att_beta.py');
    end
    delete(tmpFile);
