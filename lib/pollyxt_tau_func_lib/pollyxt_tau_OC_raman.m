function [aerBsc355_raman, aerBsc532_raman, aerBsc1064_raman, aerExt355_raman, aerExt532_raman, aerExt1064_raman, LR355_raman, LR532_raman, LR1064_raman] = pollyxt_tau_OC_raman(data, config)
%pollyxt_tau_OC_raman Retrieve aerosol optical properties for overlap corrected total and raman signal with raman method
%   Example:
%       [aerBsc355_raman, aerBsc532_raman, aerBsc1064_raman, aerExt355_raman, aerExt532_raman, aerExt1064_raman, aerLR355_raman, aerLR532_raman, aerLR1064_raman] = pollyxt_tau_OC_raman(data, config)
%   Inputs:
%       data.struct
...
for iGroup = 1:size(data.cloudFreeGroups, 1)
    ...
    sig355 = transpose(squeeze(sum(data.signal355OverlapCor(:, proIndx_387On), 2)));
    bg355 = transpose(squeeze(sum(data.bg355OverlapCor(:, proIndx_387On), 2)));
    sig387 = transpose(squeeze(sum(data.signal387OverlapCor(:, proIndx_387On), 2)));
    bg387 = transpose(squeeze(sum(data.bg387OverlapCor(:, proIndx_387On), 2)));
...
for iGroup = 1:size(data.cloudFreeGroups, 1)
    ...
    sig532 = transpose(squeeze(sum(data.signal532OverlapCor(:, proIndx_607On), 2)));
    bg532 = transpose(squeeze(sum(data.bg532OverlapCor(:, proIndx_607On), 2)));
    sig607 = transpose(squeeze(sum(data.signal607OverlapCor(:, proIndx_607On), 2)));
    bg607 = transpose(squeeze(sum(data.bg607OverlapCor(:, proIndx_607On), 2)));
