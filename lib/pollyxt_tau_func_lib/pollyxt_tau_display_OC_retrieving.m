function [] = pollyxt_tau_display_OC_retrieving(data, taskInfo, config)
%pollyxt_tau_display_OC_retrieving display aerosol optical products
%   Example:
...
for iGroup = 1:size(data.cloudFreeGroups, 1)
    ...
    elseif strcmpi(processInfo.visualizationMode, 'python')
        ...
        flag = system(sprintf('%s %s %s %s', fullfile(processInfo.pyBinDir, 'python'), fullfile(pyFolder, 'pollyxt_tau_display_OC_retrieving.py'), tmpFile, saveFolder));
        if flag ~= 0
            warning('Error in executing %s', 'pollyxt_tau_display_OC_retrieving.py');
        end
        delete(tmpFile);
    else
