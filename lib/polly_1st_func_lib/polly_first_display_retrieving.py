...
def polly_first_display_retrieving(tmpFile, saveFolder):
    """
    Description
    -----------
    ...
    Usage
    -----
    polly_first_display_retrieving(tmpFile, saveFolder)
    ...
    fig.savefig(
        os.path.join(
            saveFolder,
            '{dataFilename}_{starttime}_{endtime}_SIG.{imgFmt}'.format(
    ...
    fig.savefig(
        os.path.join(
            saveFolder,
            '{dataFilename}_{starttime}_{endtime}_Bsc_Klett.{imgFmt}'.format(
                dataFilename=rmext(dataFilename),
                starttime=datenum_to_datetime(starttime).strftime('%H%M'),
                endtime=datenum_to_datetime(endtime).strftime('%H%M'),
                imgFmt=imgForamt)),
    ...
    fig.savefig(
        os.path.join(
            saveFolder,
            '{dataFilename}_{starttime}_{endtime}_Bsc_Raman.{imgFmt}'.format(
                dataFilename=rmext(dataFilename),
                starttime=datenum_to_datetime(starttime).strftime('%H%M'),
                endtime=datenum_to_datetime(endtime).strftime('%H%M'),
                imgFmt=imgForamt)),
    ...
    fig.savefig(
        os.path.join(
            saveFolder,
            '{dataFilename}_{starttime}_{endtime}_Ext_Klett.{imgFmt}'.format(
                dataFilename=rmext(dataFilename),
                starttime=datenum_to_datetime(starttime).strftime('%H%M'),
                endtime=datenum_to_datetime(endtime).strftime('%H%M'),
                imgFmt=imgForamt)),
    ...
    fig.savefig(
        os.path.join(
            saveFolder,
            '{dataFilename}_{starttime}_{endtime}_Ext_Raman.{imgFmt}'.format(
                dataFilename=rmext(dataFilename),
                starttime=datenum_to_datetime(starttime).strftime('%H%M'),
                endtime=datenum_to_datetime(endtime).strftime('%H%M'),
                imgFmt=imgForamt)),
    ...
    fig.savefig(
        os.path.join(
            saveFolder,
            '{dataFilename}_{starttime}_{endtime}_LR_Raman.{imgFmt}'.format(
                dataFilename=rmext(dataFilename),
                starttime=datenum_to_datetime(starttime).strftime('%H%M'),
                endtime=datenum_to_datetime(endtime).strftime('%H%M'),
                imgFmt=imgForamt)),
    ...
    fig.savefig(
        os.path.join(
            saveFolder,
            '{dataFilename}_{starttime}_{endtime}_Meteor_T.{imgFmt}'.format(
                dataFilename=rmext(dataFilename),
                starttime=datenum_to_datetime(starttime).strftime('%H%M'),
                endtime=datenum_to_datetime(endtime).strftime('%H%M'),
                imgFmt=imgForamt)),
    ...
    fig.savefig(
        os.path.join(
            saveFolder,
            '{dataFilename}_{starttime}_{endtime}_Meteor_P.{imgFmt}'.format(
                dataFilename=rmext(dataFilename),
                starttime=datenum_to_datetime(starttime).strftime('%H%M'),
                endtime=datenum_to_datetime(endtime).strftime('%H%M'),
                imgFmt=imgForamt)),
...
def main():
    polly_first_display_retrieving(
        'C:\\Users\\zhenping\\Desktop\\Picasso\\tmp\\tmp.mat',
        'C:\\Users\\zhenping\\Desktop\\Picasso\\recent_plots' +
        '\\polly_first\\20180517')


if __name__ == '__main__':
    # main()
    polly_first_display_retrieving(sys.argv[1], sys.argv[2])
