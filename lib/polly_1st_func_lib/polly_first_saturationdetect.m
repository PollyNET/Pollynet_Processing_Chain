function [flag] = polly_first_saturationdetect(data, config)
%POLLY_IFT_SATURATIONDETECT detect the bins which is fully saturated by the
%clouds.
%Example:
%   [flag] = polly_ift_saturationdetect(data, height, config)
%Inputs:
%   data: struct
...
signalPCR = squeeze(data.signal + data.bg) ./ ...
    repmat(reshape(data.mShots, size(data.mShots, 1), 1, size(data.mShots, 2)), [1, size(data.signal, 2), 1]) * 150.0 ./ data.hRes;
