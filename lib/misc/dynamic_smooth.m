function [c,ww] = dynamic_smooth(varargin)
%SMOOTH  Smooth data.
...
%           'dynamic_window' - Linearly increasing window, full bins
%           'dynamic_window2' - Linearly increasing window, partial bins
...
% sets the rate of increase of the window size
slope = 0.01;
...
switch method
    ...
    case 'dynamic_window'
        c(ok) = dynamic_window(x(ok),y(ok),span,slope);
    case 'dynamic_window2'
        c(ok) = dynamic_window2(x(ok),y(ok),span,slope);
...
%%
function c = dynamic_window(y,span,slope)
%disp(size(y));
if length(y)<10
    c=y;
else
%Linearly increasing averaging window, only full bins

%bins parameter determines the initial target avergaing bins to right and
%left, window=2*size+1
bins=(span-1)/2;
%slope parameter determines the bins increment has to be between 0 and 1
%slope = slope;

%calculate height-bin (xmax) where max window size (bmax) is reached
%length(y)-bmax=size+floor(slope*(bmax-size-1))
xmax=ceil((length(y)+slope+bins*(slope-1))/(slope+1));
bmax=length(y)-xmax;

%region 1: increase bins up to target value
reg1y = cumsum(y(1:2*bins+1));
reg1y = reg1y(1:2:end);
reg1x = (1:2:2*bins+1);

%region 2: increase bins by slope up to xmax/bmax
%filter determines when to increase the bins
filter = floor(slope*(1:xmax-bins-2))-floor(slope*(0:xmax-bins-3));

reg2x = 2*floor(bins+slope*(1:xmax-bins-2))+1;
reg2y = NaN(1,length(reg2x));
...
for i = bins+2:xmax-1
   ...
   if filter(i-bins-1)
       ...
       csum = csum - y(i-cbins-1);
   end
   reg2y(i-bins-1)=csum;
end

%region 3: decrease
reg3y = cumsum(y(length(y):-1:xmax-bmax));
reg3y = reg3y(end:-2:1);
reg3x = 2*(bmax:-1:0)+1;

%mean and combine
c=[(reg1y(:)./reg1x(:)).', (reg2y(:)./reg2x(:)).', (reg3y(:)./reg3x(:)).'].';
end

%%
function c = dynamic_window2(y,span,slope)
%disp(size(y));
if length(y)<10
    c=y;
else
%Linearly increasing averaging window, also partial bins
...
%region 2: increase bins by slope up to xmax
reg2x = 2*(bins+slope*(1:xmax-bins-2))+1;
reg2y = NaN(1,length(reg2x));
...
factor = 0;
for i = bins+2:xmax-1
   csum = csum - y(i-cbins-1) + y(i+cbins);
   factor=factor+slope;
   % bins are increased when factor exceeds 1
   if floor(factor)==1
       factor = factor - 1;
       cbins = cbins+1;
       csum = csum + y(i-cbins) + y(i+cbins);
   end
   reg2y(i-bins-1)=csum + factor * (y(i-cbins-1) + y(i+cbins+1));
end
