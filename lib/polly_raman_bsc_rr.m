function [ beta_aer, LR ] = polly_raman_bsc(height, sigElastic, ...
    sigVRN2, ext_aer, angstroem, ext_mol, beta_mol, ...
    HRef, wavelength, betaRef, window_size, ...
    flagSmoothBefore)
%POLLY_RAMAN_BSC calculate the aerosol backscatter coefficient
%with Raman method.
%   Usage:
%       [ beta_aer, LR ] = Polly_raman_bsc(height, sigElastic, ...
%        sigVRN2, ext_aer, ext_mol, beta_mol, HRef, wavelength, betaRef, window_size)
%   Inputs:
%       height: array
%           height. [m]
%       sigElastic: array
...
HRefIndx = [fix((HRef(1) - height(1)) / dH) + 1, fix((HRef(end) - height(1)) / dH) + 1];
...
if wavelength == 532
    ...
    elMean = sigElastic(hIndx) .* height(hIndx).^2 ./ (beta_mol(hIndx) + betaRef);
