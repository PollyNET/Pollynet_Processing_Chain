function [LC355, LC532, LC1064, LC387, LC607, LCStd355, LCStd532, LCStd1064, LCStd387, LCStd607] = pollyxt_lacros_read_history_LC(thisTime, LCFile, config)
%pollyxt_lacros_read_history_LC read history Lidar constant from lidar constant file.
%   Example:
%       [LC355, LC532, LC1064, LC387, LC607, LCStd355, LCStd532, LCStd1064, LCStd387, LCStd607] = pollyxt_lacros_read_history_LC(thisTime, LCFile, config)
%   Inputs:
%       thisTime: datenum
%           current time. 
%       LCFile: char
...
flagValid = (LCTime > (thisTime - datenum(0,1,7))) & (LCTime < (thisTime + datenum(0,1,7))) & (LC355Status == 2);
if sum(flagValid) ~= 0

    LCTimeValid = LCTime(flagValid);
    LC355HistoryValid = LC355History(flagValid);
    LCStd355HistoryValid = LCStd355History(flagValid);
    ...
    LC355 = LC355HistoryValid(indx);
    LCStd355 = LCStd355HistoryValid(indx);
end
...
%% 1064 nm
% find the most closest calibrated value in the +- 1 week with Raman method (status=2)
flagValid = (LCTime > (thisTime - datenum(0,1,7))) & (LCTime < (thisTime + datenum(0,1,7))) & (LC1064Status == 2);
if sum(flagValid) ~= 0

    LCTimeValid = LCTime(flagValid);
    LC1064HistoryValid = LC1064History(flagValid);
    LCStd1064HistoryValid = LCStd1064History(flagValid);

    % find closest calibration results. If there are multi-values, choose the last one which is the lastest calibrated results.
    thisLag = abs(LCTimeValid - thisTime);
    minLag = min(thisLag);
    indx = find(thisLag == minLag, 1, 'last');
    LC1064 = LC1064HistoryValid(indx);
    LCStd1064 = LCStd1064HistoryValid(indx);
end

%% 387 nm
% find the most closest calibrated value in the +- 1 week with Raman method (status=2)
flagValid = (LCTime > (thisTime - datenum(0,1,7))) & (LCTime < (thisTime + datenum(0,1,7))) & (LC387Status == 2);
if sum(flagValid) ~= 0

    LCTimeValid = LCTime(flagValid);
    LC387HistoryValid = LC387History(flagValid);
    LCStd387HistoryValid = LCStd387History(flagValid);

    % find closest calibration results. If there are multi-values, choose the last one which is the lastest calibrated results.
    thisLag = abs(LCTimeValid - thisTime);
    minLag = min(thisLag);
    indx = find(thisLag == minLag, 1, 'last');
    LC387 = LC387HistoryValid(indx);
    LCStd387 = LCStd387HistoryValid(indx);
end
