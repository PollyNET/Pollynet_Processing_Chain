...
def pollyxt_lacros_display_monitor(tmpFile, saveFolder):
    '''
    Description
    -----------
    ...
    Usage
    -----
    pollyxt_lacros_display_monitor(tmpFile)
    ...
    if AD.size != 0:
        if AD[0][0] > 990:
            ...
            ax1.set_ylabel("AD [au]", fontweight='semibold', fontsize=20)
        else:
            ax1.plot(time, EN)
            # ax1.set_ylim([420, 550])
            ax1.set_ylabel("EN [mJ]", fontweight='semibold', fontsize=20)
    else:
        ax1.plot(time, EN)
        # ax1.set_ylim([420, 550])
        ax1.set_ylabel("EN [mJ]", fontweight='semibold', fontsize=20)
    ...
    ax2.set_ylabel("ExtPyro [mJ]", fontweight='semibold', fontsize=20)
    ax2.grid(True)
    ...
    ax3.set_ylim([0, 40])
    ax3.grid(True)
    ax3.set_ylabel(r'Temperature [$^\circ C$]', fontweight='semibold', fontsize=20)
    ...
    # ax4.set_ylim([-38, -20])
    ax4.grid(True)
    ax4.set_ylabel(r'Temp 1064 [$^\circ C$]', fontweight='semibold', fontsize=20)
    ax4.set_xlim([mTime[0], mTime[-1]])

    if len(time):
        cmap = ListedColormap(['navajowhite', 'coral', 'skyblue', 'm', 'mediumaquamarine'])
        pcmesh = ax5.pcolormesh(np.transpose(time), np.arange(flags.shape[0] + 1), flags, cmap=cmap, vmin=-0.5, vmax=4.5)
    ...
    ax5.set_xlabel('UTC', fontweight='semibold', fontsize=20)
...
def main():
    pollyxt_lacros_display_monitor('C:\\Users\\zhenping\\Desktop\\Picasso\\tmp\\tmp.mat', 'C:\\Users\\zhenping\\Desktop\\Picasso\\recent_plots\\POLLYXT_lacros\\20180517')

if __name__ == '__main__':
    # main()
    pollyxt_lacros_display_monitor(sys.argv[1], sys.argv[2])