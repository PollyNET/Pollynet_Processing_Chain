function [] = pollyxt_lacros_save_quasi_results(data, taskInfo, config)
%pollyxt_lacros_save_quasi_results Saving the target classification results to netcdf file.
%   Example:
%       [] = pollyxt_lacros_save_quasi_results(data, config)
%   Inputs:
%		data: struct
...
varID_quasi_bsc_1064 = netcdf.defVar(ncID, 'quasi_bsc_1064', 'NC_DOUBLE', [dimID_altitude, dimID_time]);
varID_quasi_pardepol_532 = netcdf.defVar(ncID, 'quasi_pardepol_532', 'NC_DOUBLE', [dimID_altitude, dimID_time]);
varID_quasi_ang_532_1064 = netcdf.defVar(ncID, 'quasi_ang_532_1064', 'NC_DOUBLE', [dimID_altitude, dimID_time]);
varID_quality_mask_532 = netcdf.defVar(ncID, 'quality_mask_532', 'NC_DOUBLE', [dimID_altitude, dimID_time]);
varID_quality_mask_1064 = netcdf.defVar(ncID, 'quality_mask_1064', 'NC_DOUBLE', [dimID_altitude, dimID_time]);
...
netcdf.putAtt(ncID, varID_quasi_bsc_1064, 'unit', 'm^{-1}*Sr^{-1}');
...
netcdf.putAtt(ncID, varID_quasi_bsc_532, 'comment', 'This parameter is retrieved by the method demonstrated in (Holger, ATM, 2017). The retrieved results are dependent on the lidar constants and the AOD below the current bin. If the AOD is greater than 0.3, the relative uncertainty can be as large as 20%. Be careful about that!');
