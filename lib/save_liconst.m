function save_liconst(dbFile, liconst, liconstStd, ...
                         ...
                         pollyDataFilename, pollyType, wavelength, caliMethod)
...
for iLC = 1:length(liconst)
    ...
    exec(conn, sprintf(['INSERT OR REPLACE INTO lidar_calibration_constant', ...
        ...
        'wavelength, nc_zip_file, polly_type, cali_method) VALUES(''%s'', ''%s'', ', ...
        '%f, %f, ''%s'', ''%s'', ''%s'', ''%s'')'], ...
