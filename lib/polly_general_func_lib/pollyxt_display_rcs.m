function pollyxt_display_rcs(data, taskInfo, config)
%POLLYXT_DISPLAY_RCS display the range corrected signal, vol-depol at 355 and 532 nm
%Example:
...
if strcmpi(processInfo.visualizationMode, 'matlab')
    ...
    fileVolDepol355 = fullfile(processInfo.pic_folder, campaignInfo.name, datestr(data.mTime(1), 'yyyy'), datestr(data.mTime(1), 'mm'), datestr(data.mTime(1), 'dd'), sprintf('%s_VDR_355.%s', rmext(taskInfo.dataFilename), imgFormat));
