...
if ~ sum(data.flagCloudFree2km) == 0
    ...
    % 387 nm
    sig387NR = squeeze(sum(data.signal(flag387NR, :, data.flagCloudFree2km), 3));
    bg387NR = squeeze(sum(data.bg(flag387NR, :, data.flagCloudFree2km), 3));
    sig387FR = squeeze(sum(data.signal(flag387FR, :, data.flagCloudFree2km), 3));
    bg387FR = squeeze(sum(data.bg(flag387FR, :, data.flagCloudFree2km), 3));
...
case {1, 2}
    ...
    data.signal355OverlapCor = OverlapCor(squeeze(data.signal(flag355FR, :, :)), ...
            transpose(overlap355Sm), data.height, data.height(normRange355));
    ...
    data.signal387OverlapCor = OverlapCor(squeeze(data.signal(flag387FR, :, :)), ...
            transpose(overlap387Sm), data.height, data.height(normRange387));
    ...
    data.signal532OverlapCor = OverlapCor(squeeze(data.signal(flag532FR, :, :)), ...
            transpose(overlap532Sm), data.height, data.height(normRange532));
    ...
    data.signal607OverlapCor = OverlapCor(squeeze(data.signal(flag607FR, :, :)), ...
            transpose(overlap607Sm), data.height, data.height(normRange607));
...
case 3
    ...
    data.signal355OverlapCor = SigGlue(squeeze(data.signal(flag355FR, :, :)), ...
                    squeeze(data.signal(flag355NR, :, :)), sigRatio355, ...
                    data.height, data.height(normRange355));
    data.bg355OverlapCor = SigGlue(squeeze(data.bg(flag355FR, :, :)), ...
                    squeeze(data.bg(flag355NR, :, :)), sigRatio355, ...
                    data.height, data.height(normRange355));
    data.signal387OverlapCor = SigGlue(squeeze(data.signal(flag387FR, :, :)), ...
                    squeeze(data.signal(flag387NR, :, :)), sigRatio387, ...
                    data.height, data.height(normRange387));
    data.bg387OverlapCor = SigGlue(squeeze(data.bg(flag387FR, :, :)), ...
                    squeeze(data.bg(flag387NR, :, :)), sigRatio387, ...
                    data.height, data.height(normRange387));

    data.signal532OverlapCor = SigGlue(squeeze(data.signal(flag532FR, :, :)), ...
                    squeeze(data.signal(flag532NR, :, :)), sigRatio532, ...
                    data.height, data.height(normRange532));
    data.bg532OverlapCor = SigGlue(squeeze(data.bg(flag532FR, :, :)), ...
                    squeeze(data.bg(flag532NR, :, :)), sigRatio532, ...
                    data.height, data.height(normRange532));
    data.signal607OverlapCor = SigGlue(squeeze(data.signal(flag607FR, :, :)), ...
                    squeeze(data.signal(flag607NR, :, :)), sigRatio607, ...
                    data.height, data.height(normRange607));
    data.bg607OverlapCor = SigGlue(squeeze(data.bg(flag607FR, :, :)), ...
                    squeeze(data.bg(flag607NR, :, :)), sigRatio607, ...
                    data.height, data.height(normRange607));
