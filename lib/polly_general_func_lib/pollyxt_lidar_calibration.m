function [LC] = pollyxt_lidar_calibration(data, config)
%POLLYXT_LIDAR_CALIBRATION calibrate the lidar system
%Example:
...
%       LC_raman_607: array
%           lidar calibration constants for 30s profile.
%       LCStd_klett_355: array
%           uncertainty of lidar constant.
%       LCStd_klett_532: array
%           uncertainty of lidar constant.
%       LCStd_klett_1064: array
%           uncertainty of lidar constant.
%       LCStd_raman_355: array
%           uncertainty of lidar constant.
%       LCStd_raman_532: array
%           uncertainty of lidar constant.
%       LCStd_raman_1064: array
%           uncertainty of lidar constant.
%       LCStd_aeronet_355: array
%           uncertainty of lidar constant.
%       LCStd_aeronet_532: array
%           uncertainty of lidar constant.
%       LCStd_aeronet_1064: array
%           uncertainty of lidar constant.
%       LCStd_raman_387: array
%           uncertainty of lidar constant.
%       LCStd_raman_607: array
%           uncertainty of lidar constant.
%       LC_start_time: array
...
LC.LCStd_raman_532 = [];
LC.LCStd_raman_1064 = [];
LC.LCStd_aeronet_355 = [];
LC.LCStd_aeronet_532 = [];
LC.LCStd_aeronet_1064 = [];
LC.LCStd_raman_607 = [];
LC.LCStd_raman_387 = [];
LC.LC_start_time = [];
LC.LC_stop_time = [];
...
if isempty(hIndxFullOverlap355)
    fprintf(['Failure in searching the index of full overlap at %s. ', ...
             'Set it to be .\nheightFullOverlap: %f\n'], ...
             config.channelTag(flagChannel355), ...
             config.heightFullOverlap(flagChannel355));
    hIndxFullOverlap355 = 70;
end
if isempty(hIndxFullOverlap532)
    fprintf(['Failure in searching the index of full overlap at %s. ', ...
             'Set it to be .\nheightFullOverlap: %f\n'], ...
             config.channelTag(flagChannel532), ...
             config.heightFullOverlap(flagChannel532));
    hIndxFullOverlap532 = 70;
end
if isempty(hIndxFullOverlap1064)
    fprintf(['Failure in searching the index of full overlap at %s. ', ...
             'Set it to be .\nheightFullOverlap: %f\n'], ...
             config.channelTag(flagChannel1064), ...
             config.heightFullOverlap(flagChannel1064));
...
for iGroup = 1:size(data.cloudFreeGroups, 1)
    ...
    if ~ isnan(data.aerBsc355_klett(iGroup, 80))
        ...
        [LC_klett_355, ~, relStd] = mean_stable(LC_klett_355_Profile, config.LCMeanWindow, config.LCMeanMinIndx, config.LCMeanMaxIndx);
        LCStd_klett_355 = LC_klett_355 * relStd;
    end

    % concatenate the results
    LC.LC_klett_355 = cat(1, LC.LC_klett_355, LC_klett_355);
    LC.LCStd_klett_355 = cat(1, LC.LCStd_klett_355, LCStd_klett_355);
    ...
    if ~ isnan(data.aerBsc1064_klett(iGroup, 80))
        ...
        [LC_klett_1064, ~, relStd] = mean_stable(LC_klett_1064_Profile, config.LCMeanWindow, config.LCMeanMinIndx, config.LCMeanMaxIndx);
        LCStd_klett_1064 = LC_klett_1064 * relStd;
    end
...
for iGroup = 1:size(data.cloudFreeGroups, 1)
    ...
    if ~ isnan(data.aerBsc355_raman(iGroup, 80))
        ...
        [LC_raman_355, ~, relStd] = mean_stable(LC_raman_355_Profile, config.LCMeanWindow, config.LCMeanMinIndx, config.LCMeanMaxIndx);
        LCStd_raman_355 = LC_raman_355 * relStd;
    end

    % concatenate the results
    LC.LC_raman_355 = cat(1, LC.LC_raman_355, LC_raman_355);
    LC.LCStd_raman_355 = cat(1, LC.LCStd_raman_355, LCStd_raman_355);
    ...
    if ~ isnan(data.aerBsc1064_raman(iGroup, 80))
        ...
        [LC_raman_1064, ~, relStd] = mean_stable(LC_raman_1064_Profile, config.LCMeanWindow, config.LCMeanMinIndx, config.LCMeanMaxIndx);
        LCStd_raman_1064 = LC_raman_1064 * relStd;
    end

    % concatenate the results
    LC.LC_raman_1064 = cat(1, LC.LC_raman_1064, LC_raman_1064);
    LC.LCStd_raman_1064 = cat(1, LC.LCStd_raman_1064, LCStd_raman_1064);
    ...
    if ~ isnan(data.aerBsc355_raman(iGroup, 80))
        ...
        [LC_raman_387, ~, relStd] = mean_stable(LC_raman_387_Profile, config.LCMeanWindow, config.LCMeanMinIndx, config.LCMeanMaxIndx);
        LCStd_raman_387 = LC_raman_387 * relStd;
    end

    % concatenate the results
    LC.LC_raman_387 = cat(1, LC.LC_raman_387, LC_raman_387);
    LC.LCStd_raman_387 = cat(1, LC.LCStd_raman_387, LCStd_raman_387);
...
for iGroup = 1:size(data.cloudFreeGroups, 1)
    ...
    LC_aeronet_532 = NaN;
    LC_aeronet_1064 = NaN;
    LCStd_aeronet_355 = NaN;
    LCStd_aeronet_532 = NaN;
    LCStd_aeronet_1064 = NaN;
    ...
    if ~ isnan(data.aerBsc355_aeronet(iGroup, 80))
        ...
        [LC_aeronet_355, ~, relStd] = mean_stable(LC_aeronet_355_Profile, config.LCMeanWindow, config.LCMeanMinIndx, config.LCMeanMaxIndx);
        LCStd_aeronet_355 = LC_aeronet_355 * relStd;
    end

    % concatenate the results
    LC.LC_aeronet_355 = cat(1, LC.LC_aeronet_355, LC_aeronet_355);
    LC.LCStd_aeronet_355 = cat(1, LC.LCStd_aeronet_355, LCStd_aeronet_355);
    ...
    if ~ isnan(data.aerBsc1064_aeronet(iGroup, 80))
        ...
        [LC_aeronet_1064, ~, relStd] = mean_stable(LC_aeronet_1064_Profile, config.LCMeanWindow, config.LCMeanMinIndx, config.LCMeanMaxIndx);
        LCStd_aeronet_1064 = LC_aeronet_1064 * relStd;
    end

    % concatenate the results
    LC.LC_aeronet_1064 = cat(1, LC.LC_aeronet_1064, LC_aeronet_1064);
    LC.LCStd_aeronet_1064 = cat(1, LC.LCStd_aeronet_1064, LCStd_aeronet_1064);
