...
%           the measurement time, which used for searching the closest sonding file.
%   Outputs:
%       sondeFile: str
%           the filename of the searched sonding file. If no file was found, an empty string will be returned.
%   History:
...
if ~ exist(sondeFolder, 'dir')
    warning('sondeFolder does not exist! Please check you set the right folder in polly config file. \n%s', sondeFolder);
    return;
end
...
if isempty(sondeFileList)
    warning('No required radiosonde files was found in the sonde folder. Please go to the folder below to have a look.\n%s', sondeFolder);
    return;
end
...
if minDeltaTime < datenum(0, 1, 1, 0, 0, 0)
    ...
    warning('There was no sonde launching within 1 day.\nThe measurement time: %s\nThe closest time of sonding: %s', datestr(measurementTime, 'yyyymmdd HH:MM:SS'), datestr(sondeTime(indxSondeFile), 'yyyymmdd HH:MM:SS'));
end
