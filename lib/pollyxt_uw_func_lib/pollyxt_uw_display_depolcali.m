function [] = pollyxt_uw_display_depolcali(data, taskInfo, attri)
%pollyxt_uw_display_depolcali display the depolarization calibration results
%   Example:
%       [] = pollyxt_uw_display_depolcali(attri)
%   Inputs:
%       attri
%   Outputs:
%       
...
if strcmpi(processInfo.visualizationMode, 'matlab')
	...
	for iCali = 1:length(attri.depCalAttri355.thisCaliTime)  
		...
		fileDepolCali355 = fullfile(processInfo.pic_folder, taskInfo.pollyVersion, datestr(data.mTime(1), 'yyyy'), datestr(data.mTime(1), 'mm'), datestr(data.mTime(1), 'dd'), sprintf('%s_%d_DepolCali_355.png', datestr(thisCaliTime, 'yyyymmdd-HHMM'), wavelength));
...
elseif strcmpi(processInfo.visualizationMode, 'python')
	...
	for iCali = 1:length(attri.depCalAttri532.thisCaliTime)  
	    ...
	    flag = system(sprintf('%s %s %s %s', fullfile(processInfo.pyBinDir, 'python'), fullfile(pyFolder, 'pollyxt_uw_display_depolcali.py'), tmpFile, saveFolder));
	    if flag ~= 0
	        warning('Error in executing %s', 'pollyxt_uw_display_depolcali.py');
	    end
