function [] = pollyxt_uw_display_rcs(data, taskInfo, config)
%pollyxt_uw_display_rcs display the range corrected signal, vol-depol at 355 and 532 nm
%   Example:
%       [] = pollyxt_uw_display_rcs(data, config)
%   Inputs:
%		data: struct
...
elseif strcmpi(processInfo.visualizationMode, 'python')
    ...
    flag = system(sprintf('%s %s %s %s', fullfile(processInfo.pyBinDir, 'python'), fullfile(pyFolder, 'pollyxt_uw_display_rcs.py'), tmpFile, saveFolder));
    if flag ~= 0
        warning('Error in executing %s', 'pollyxt_uw_display_rcs.py');
    end
