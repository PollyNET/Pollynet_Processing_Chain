...
data.LCUsed = pollyxt_ift_select_liconst(data, config, dbFile);
fprintf('[%s] Finish.\n', tNow());
...
if processInfo.flagEnableResultsOutput
    ...
    for iProd = 1:length(config.prodSaveList)
        ...
        case 'aerproffr'
            ...
            pollyxt_ift_save_retrieving_results_no_wv(data, taskInfo, config);
...
if processInfo.flagEnableDataVisualization
    ...
    pollyxt_ift_display_retrieving_no_wv(data, taskInfo, config);
...
%% get report
report = pollyxt_ift_results_report_no_wv(data, taskInfo, config);
