function [reportStr] = pollyxt_ift_results_report(data, taskInfo, config)
%pollyxt_ift_results_report Write the info to done list file and generate the report for the current task. These report can be used for further examination.
%   Example:
%       [reportStr] = pollyxt_ift_results_report(data, taskInfo, config)
%   Inputs:
...
write_2_donelist(processInfo.doneListFile, 'a', campaignInfo.name, campaignInfo.location, datestr(data.mTime(1), 'yyyymmdd HH:MM:SS'), datestr(data.mTime(end), 'yyyymmdd HH:MM:SS'), datestr(taskInfo.startTime, 'yyyymmdd HH:MM:SS'), '407', fullfile('recent_plots_new', campaignInfo.name, datestr(data.mTime(1), 'yyyy'), datestr(data.mTime(1), 'mm'), datestr(data.mTime(1), 'dd'), sprintf('%s_SAT_FR_1064.png', rmext(taskInfo.dataFilename))), '0', sprintf('signal status at 407 nm. SNR threshold is %d', config.mask_SNRmin(flag1064FR)), taskInfo.zipFile, num2str(taskInfo.dataSize), num2str(active), num2str(data.quasiAttri.flagGDAS1), datestr_convert_0(data.quasiAttri.timestamp, 'yyyymmdd HH:MM:SS'), '50', processInfo.programVersion, 'SAT_FR_1064', datestr(data.mTime(1), 'yyyymmdd HH:MM:SS'), datestr(data.mTime(end), 'yyyymmdd HH:MM:SS'));
