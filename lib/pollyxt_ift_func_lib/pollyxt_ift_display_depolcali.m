function pollyxt_ift_display_depolcali(data, config, taskInfo, attri)
%POLLYXT_IFT_DISPLAY_DEPOLCALI display the depolarization calibration results
%Example:
%   pollyxt_ift_display_depolcali(data, config, taskInfo, attri)
%Inputs:
%   data, taskInfo, attri
%History:
...
elseif strcmpi(processInfo.visualizationMode, 'python')
    ...
    for iCali = 1:length(attri.depCalAttri532.caliTime)  
        ...
        flag = system(sprintf('%s %s %s %s', fullfile(processInfo.pyBinDir, 'python'), fullfile(pyFolder, 'pollyxt_ift_display_depolcali.py'), tmpFile, saveFolder));
        if flag ~= 0
            warning('Error in executing %s', 'pollyxt_ift_display_depolcali.py');
        end
        delete(tmpFile);
    end
