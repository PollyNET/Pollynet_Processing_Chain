function pollyxt_ift_display_rcs_UV(data, taskInfo, config)
%POLLYXT_IFT_DISPLAY_RCS_UV display the range corrected signal, vol-depol at 355 and 532 nm
%Example:
%   pollyxt_ift_display_rcs_UV(data, config)
%Inputs:
%   data.struct
...
elseif strcmpi(processInfo.visualizationMode, 'python')
    ...
    save(tmpFile, 'figDPI', 'mTime', 'height', 'depCalMask', 'fogMask', 'yLim_FR_RCS', 'yLim_NR_RCS', 'yLim_FR_DR', 'RCS_FR_355', 'RCS_FR_532', 'RCS_FR_1064', 'RCS_NR_355', 'RCS_NR_532', 'volDepol_355', 'processInfo', 'campaignInfo', 'taskInfo', 'xtick', 'xtickstr', 'RCS355FRColorRange', 'RCS532FRColorRange', 'RCS1064FRColorRange', 'RCS355NRColorRange', 'RCS532NRColorRange', 'Voldepol355ColorRange', 'imgFormat', 'colormap_basic', 'flagWatermarkOn', 'partnerLabel', '-v6');
    flag = system(sprintf('%s %s %s %s', fullfile(processInfo.pyBinDir, 'python'), fullfile(pyFolder, 'pollyxt_ift_display_rcs_UV.py'), tmpFile, saveFolder));
    if flag ~= 0
        warning('Error in executing %s', 'pollyxt_ift_display_rcs_UV.py');
    end
    delete(tmpFile);
