function pollyxt_ift_save_retrieving_results_UV(data, taskInfo, config)
%POLLYXT_IFT_SAVE_RETRIEVING_RESULTS_UV saving the retrieved results, including backscatter, extinction coefficients, lidar ratio, volume/particles depolarization ratio and so on.
%Example:
...
for iGroup = 1:size(data.cloudFreeGroups, 1)
    ...
    netcdf.putAtt(ncID, varID_volDepol_klett_355, 'retrieved_info', sprintf('Smoothing window: %d [m];', config.smoothWin_klett_355 * data.hRes));
    ...
    netcdf.putAtt(ncID, varID_volDepol_raman_355, 'retrieved_info', sprintf('Smoothing window: %d [m];', config.smoothWin_raman_355 * data.hRes));
    ...
    netcdf.putAtt(ncID, varID_parDepol_klett_355, 'retrieved_info', sprintf('Smoothing window: %d [m]', config.smoothWin_klett_355 * data.hRes));
    ...
    netcdf.putAtt(ncID, varID_parDepol_raman_355, 'retrieved_info', sprintf('Smoothing window: %d [m]', config.smoothWin_raman_355 * data.hRes));
    ...
    netcdf.putAtt(ncID, varID_parDepol_std_raman_355, 'retrieving_info', sprintf('Smoothing window: %d [m]', config.smoothWin_raman_532 * data.hRes));
