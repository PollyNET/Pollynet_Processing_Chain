function [att_beta_355, att_beta_532, att_beta_1064, att_beta_387, att_beta_607] = pollyxt_ift_att_beta(data, config)
%pollyxt_ift_att_beta Calculate the attenuated backscatter.
%   Example:
...
RCS355 = squeeze(data.signal(flagChannel355, :, :)) .* repmat(transpose(data.height), 1, numel(data.mTime)).^2;
...
RCS1064 = squeeze(data.signal(flagChannel1064, :, :)) .* repmat(transpose(data.height), 1, numel(data.mTime)).^2;
RCS1064(:, data.depCalMask) = NaN;
RCS387 = squeeze(data.signal(flagChannel387, :, :)) .* repmat(transpose(data.height), 1, numel(data.mTime)).^2;
