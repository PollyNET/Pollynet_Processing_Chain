function pollyxt_ift_display_depolcali_UV(data, config, taskInfo, attri)
%POLLYXT_IFT_DISPLAY_DEPOLCALI_UV display the depolarization calibration results
%Example:
%   pollyxt_ift_display_depolcali_UV(data, config, taskInfo, attri)
%Inputs:
%   data, taskInfo, attri
%History:
...
if strcmpi(processInfo.visualizationMode, 'matlab')
    ...
    for iCali = 1:length(attri.depCalAttri355.caliTime)  
        ...
        fileDepolCali355 = fullfile(processInfo.pic_folder, campaignInfo.name, datestr(data.mTime(1), 'yyyy'), datestr(data.mTime(1), 'mm'), datestr(data.mTime(1), 'dd'), sprintf('%s_%d_DepolCali_355.%s', datestr(caliTime, 'yyyymmdd-HHMM'), wavelength, imgFormat));
...
elseif strcmpi(processInfo.visualizationMode, 'python')
    ...
    for iCali = 1:length(attri.depCalAttri355.caliTime)  
        ...
        save(tmpFile, 'figDPI', 'wavelength', 'time', 'height', 'sig_t_p', 'sig_t_m', 'sig_x_p', 'sig_x_m', 'caliHIndxRange', 'indx_45m', 'indx_45p', 'dplus', 'dminus', 'segmentLen', 'indx', 'mean_dplus_tmp', 'std_dplus_tmp', 'mean_dminus_tmp', 'std_dminus_tmp', 'TR_t', 'TR_x', 'segIndx', 'caliTime', 'processInfo', 'campaignInfo', 'taskInfo', 'imgFormat', 'flagWatermarkOn', 'partnerLabel', '-v6');
        flag = system(sprintf('%s %s %s %s', fullfile(processInfo.pyBinDir, 'python'), fullfile(pyFolder, 'pollyxt_ift_display_depolcali_UV.py'), tmpFile, saveFolder));
        if flag ~= 0
            warning('Error in executing %s', 'pollyxt_ift_display_depolcali_UV.py');
        end
        delete(tmpFile);
    end
