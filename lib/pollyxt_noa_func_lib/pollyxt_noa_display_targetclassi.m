function [] = pollyxt_noa_display_targetclassi(data, taskInfo, config)
%pollyxt_noa_display_targetclassi display the target classification reuslts
%   Example:
...
elseif strcmpi(processInfo.visualizationMode, 'python')
    ...
    flag = system(sprintf('%s %s %s %s', fullfile(processInfo.pyBinDir, 'python'), fullfile(pyFolder, 'pollyxt_noa_display_targetclassi.py'), tmpFile, saveFolder));
    if flag ~= 0
        warning('Error in executing %s', 'pollyxt_noa_display_targetclassi.py');
    end
