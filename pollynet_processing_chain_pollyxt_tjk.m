function [report] = pollynet_processing_chain_tjk(taskInfo, config)
%POLLYNET_PROCESSING_CHAIN_tjk processing the data from pollyxt_tjk
%   Example:
%       [report] = pollynet_processing_chain_tjk(taskInfo, config)
%   Inputs:
%       taskInfo, config
%   Outputs:
%       report: cell array
...
[data, overlapAttri] = pollyxt_tjk_overlap(data, config);
fprintf('[%s] Finish.\n', tNow());

%% split the cloud free profiles into continuous subgroups
fprintf('\n[%s] Start to split the cloud free profiles.\n', tNow());
cloudFreeGroups = pollyxt_tjk_splitcloudfree(data, config);
...
[temperature, pressure, relh, meteorAttri] = pollyxt_tjk_readmeteor(data, config);
...
[data.refHIndx355, data.refHIndx532, data.refHIndx1064, data.dpIndx355, data.dpIndx532, data.dpIndx1064] = pollyxt_tjk_rayleighfit(data, config);
...
% Klett method 
[data.el355, data.bgEl355, data.el532, data.bgEl532] = pollyxt_tjk_transratioCor(data, config);
[data.aerBsc355_klett, data.aerBsc532_klett, data.aerBsc1064_klett, data.aerExt355_klett, data.aerExt532_klett, data.aerExt1064_klett] = pollyxt_tjk_klett(data, config);
[data.aerBsc355_NR_klett, data.aerBsc532_NR_klett, data.aerExt355_NR_klett, data.aerExt532_NR_klett, data.refBeta_NR_355_klett, data.refBeta_NR_532_klett] = pollyxt_tjk_NR_klett(data, config);

% Constrained-AOD Klett method
[data.aerBsc355_aeronet, data.aerBsc532_aeronet, data.aerBsc1064_aeronet, data.aerExt355_aeronet, data.aerExt532_aeronet, data.aerExt1064_aeronet, data.LR355_aeronet, data.LR532_aeronet, data.LR1064_aeronet, data.deltaAOD355, data.deltaAOD532, data.deltaAOD1064] = pollyxt_tjk_constrainedklett(data, AERONET, config);   % constrain Lidar Ratio

% Raman method
[data.aerBsc355_raman, data.aerBsc532_raman, data.aerBsc1064_raman, data.aerExt355_raman, data.aerExt532_raman, data.aerExt1064_raman, data.LR355_raman, data.LR532_raman, data.LR1064_raman] = pollyxt_tjk_raman(data, config);
[data.aerBsc355_NR_raman, data.aerBsc532_NR_raman, data.aerExt355_NR_raman, data.aerExt532_NR_raman, data.LR355_NR_raman, data.LR532_NR_raman, data.refBeta_NR_355_raman, data.refBeta_NR_532_raman] = pollyxt_tjk_NR_raman(data,config);

% Vol- and Par-depol
[data.voldepol355_klett, data.pardepol355_klett, data.pardepolStd355_klett, data.voldepol355_raman, data.pardepol355_raman, data.pardepolStd355_raman, data.moldepol355, data.moldepolStd355, data.flagDefaultMoldepol355, data.voldepol532_klett, data.pardepol532_klett, data.pardepolStd532_klett, data.voldepol532_raman, data.pardepol532_raman, data.pardepolStd532_raman, data.moldepol532, data.moldepolStd532, data.flagDefaultMoldepol532] = pollyxt_tjk_depolratio(data, config);

% Angstroem exponent
[data.ang_ext_355_532_raman, data.ang_bsc_355_532_raman, data.ang_bsc_532_1064_raman, data.ang_bsc_355_532_klett, data.ang_bsc_532_1064_klett] = pollyxt_tjk_angstrexp(data, config);
[data.ang_ext_355_532_raman_NR, data.ang_bsc_355_532_raman_NR, data.ang_bsc_355_532_klett_NR] = pollyxt_tjk_NR_angstrexp(data, config);
fprintf('[%s] Finish.\n', tNow());

%% water vapor calibration
% get IWV from other instruments
fprintf('\n[%s] Start to water vapor calibration.\n', tNow());
[data.IWV, IWVAttri] = pollyxt_tjk_read_IWV(data, config);
data.IWVAttri = IWVAttri;
[wvconst, wvconstStd, wvCaliInfo] = pollyxt_tjk_wv_calibration(data, config);
% if not successful wv calibration, choose the default values
[data.wvconstUsed, data.wvconstUsedStd, data.wvconstUsedInfo] = pollyxt_tjk_select_wvconst(wvconst, wvconstStd, data.IWVAttri, polly_parsetime(taskInfo.dataFilename, config.dataFileFormat), fullfile(processInfo.results_folder, campaignInfo.name, config.wvCaliFile), config.flagUsePreviousLC);
[data.wvmr, data.rh, ~, data.WVMR, data.RH] = pollyxt_tjk_wv_retrieve(data, config, wvCaliInfo.IntRange);
fprintf('[%s] Finish.\n', tNow());
...
data.LC = LC;
LCUsed = struct();
[LCUsed.LCUsed355, LCUsed.LCUsedTag355, LCUsed.flagLCWarning355, LCUsed.LCUsed532, LCUsed.LCUsedTag532, LCUsed.flagLCWarning532, LCUsed.LCUsed1064, LCUsed.LCUsedTag1064, LCUsed.flagLCWarning1064, LCUsed.LCUsed387, LCUsed.LCUsedTag387, LCUsed.flagLCWarning387, LCUsed.LCUsed607, LCUsed.LCUsedTag607, LCUsed.flagLCWarning607] = pollyxt_tjk_mean_LC(data, config, taskInfo, fullfile(processInfo.results_folder, config.pollyVersion));
data.LCUsed = LCUsed;
fprintf('[%s] Finish.\n', tNow());

%% attenuated backscatter
fprintf('\n[%s] Start to calculate attenuated backscatter.\n', tNow());
[att_beta_355, att_beta_532, att_beta_1064, att_beta_387, att_beta_607] = pollyxt_tjk_att_beta(data, config);
...
tc_mask_V2 = pollyxt_tjk_targetclassi_V2(data, config);
...
if processInfo.flagEnableResultsOutput
    ...
    pollyxt_tjk_save_NR_retrieving_results(data, taskInfo, config);
    ...
    pollyxt_tjk_save_WVMR_RH(data, taskInfo, config);
    
    %% save volume depolarization ratio
    pollyxt_tjk_save_voldepol(data, taskInfo, config);

    %% save quasi results
    pollyxt_tjk_save_quasi_results(data, taskInfo, config);

    %% save quasi results V2
    pollyxt_tjk_save_quasi_results_V2(data, taskInfo, config);

    %% save target classification results
    pollyxt_tjk_save_tc(data, taskInfo, config);

    %% save target classification results V2
    pollyxt_tjk_save_tc_V2(data, taskInfo, config);
...
if processInfo.flagEnableDataVisualization
    ...
    pollyxt_tjk_display_monitor(data, taskInfo, config);

    %% display signal
    disp('Display RCS and volume depolarization ratio')
    pollyxt_tjk_display_rcs(data, taskInfo, config);

    %% display depol calibration results
    disp('Display depolarization calibration results')
    pollyxt_tjk_display_depolcali(data, taskInfo, depCaliAttri);

    %% display saturation and cloud free tags
    disp('Display signal flags')
    pollyxt_tjk_display_saturation(data, taskInfo, config);
...
%% get report
report = pollyxt_tjk_results_report(data, taskInfo, config);
