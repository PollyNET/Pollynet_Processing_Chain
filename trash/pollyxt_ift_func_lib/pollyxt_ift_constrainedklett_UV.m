function [aerBsc355_aeronet, aerBsc532_aeronet, aerBsc1064_aeronet, aerExt355_aeronet, aerExt532_aeronet, aerExt1064_aeronet, LR355_aeronet, LR532_aeronet, LR1064_aeronet, deltaAOD355, deltaAOD532, deltaAOD1064] = pollyxt_ift_constrainedklett_UV(data, AERONET, config)
%POLLYXT_IFT_CONSTRAINEDKLETT_UV Retrieve the aerosol optical properties with the constrains from AERONET.
%Example:
%   [aerBsc355_aeronet, aerBsc532_aeronet, aerBsc1064_aeronet, aerExt355_aeronet, aerExt532_aeronet, aerExt1064_aeronet, LR355_aeronet, LR532_aeronet, LR1064_aeronet] = pollyxt_ift_constrainedklett_UV(data, AERONET, config)
%Inputs:
%   data.struct
...
for iGroup = 1:size(data.cloudFreeGroups, 1)
    ...
    if ~ isnan(data.refHIndx532(iGroup, 1))
        ...
        if (~ isempty(AERONETIndx)) && (snrRef532 >= config.minRefSNR532)
            AOD_532_aeronet = interp_AERONET_AOD(340, AERONET.AOD_340(AERONETIndx), 380, AERONET.AOD_380(AERONETIndx), 532);
...
for iGroup = 1:size(data.cloudFreeGroups, 1)
    ...
    if ~ isnan(data.refHIndx355(iGroup, 1))
        ...
        if (~ isempty(AERONETIndx)) && (snrRef355 >= config.minRefSNR355)
            AOD_355_aeronet = interp_AERONET_AOD(500, AERONET.AOD_500(AERONETIndx), 675, AERONET.AOD_675(AERONETIndx), 355);
