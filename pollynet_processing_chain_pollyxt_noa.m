function [report] = pollynet_processing_chain_pollyxt_noa(taskInfo, config)
%POLLYNET_PROCESSING_CHAIN_pollyxt_noa processing the data from pollyxt_noa
%	Example:
%		[report] = pollynet_processing_chain_pollyxt_noa(taskInfo, config)
%	Inputs:
%		taskInfo, config
%	Outputs:
%		report: cell array
...
[data, overlapAttri] = pollyxt_noa_overlap(data, config);
fprintf('[%s] Finish.\n', tNow());

%% split the cloud free profiles into continuous subgroups
fprintf('\n[%s] Start to split the cloud free profiles.\n', tNow());
cloudFreeGroups = pollyxt_noa_splitcloudfree(data, config);
...
[temperature, pressure, relh, meteorAttri] = pollyxt_noa_readmeteor(data, config);
...
[data.refHIndx355, data.refHIndx532, data.refHIndx1064, data.dpIndx355, data.dpIndx532, data.dpIndx1064] = pollyxt_noa_rayleighfit(data, config);
...
% Klett method 
[data.el355, data.bgEl355, data.el532, data.bgEl532] = pollyxt_noa_transratioCor(data, config);
[data.aerBsc355_klett, data.aerBsc532_klett, data.aerBsc1064_klett, data.aerExt355_klett, data.aerExt532_klett, data.aerExt1064_klett] = pollyxt_noa_klett(data, config);
[data.aerBsc355_NR_klett, data.aerBsc532_NR_klett, data.aerExt355_NR_klett, data.aerExt532_NR_klett, data.refBeta_NR_355_klett, data.refBeta_NR_532_klett] = pollyxt_noa_NR_klett(data, config);

% Constrained-AOD Klett method
[data.aerBsc355_aeronet, data.aerBsc532_aeronet, data.aerBsc1064_aeronet, data.aerExt355_aeronet, data.aerExt532_aeronet, data.aerExt1064_aeronet, data.LR355_aeronet, data.LR532_aeronet, data.LR1064_aeronet, data.deltaAOD355, data.deltaAOD532, data.deltaAOD1064] = pollyxt_noa_constrainedklett(data, AERONET, config);   % constrain Lidar Ratio

% Raman method
[data.aerBsc355_raman, data.aerBsc532_raman, data.aerBsc1064_raman, data.aerExt355_raman, data.aerExt532_raman, data.aerExt1064_raman, data.LR355_raman, data.LR532_raman, data.LR1064_raman] = pollyxt_noa_raman(data, config);
[data.aerBsc355_NR_raman, data.aerBsc532_NR_raman, data.aerExt355_NR_raman, data.aerExt532_NR_raman, data.LR355_NR_raman, data.LR532_NR_raman, data.refBeta_NR_355_raman, data.refBeta_NR_532_raman] = pollyxt_noa_NR_raman(data,config);

% Vol- and Par-depol
[data.voldepol355_klett, data.pardepol355_klett, data.pardepolStd355_klett, data.voldepol355_raman, data.pardepol355_raman, data.pardepolStd355_raman, data.moldepol355, data.moldepolStd355, data.flagDefaultMoldepol355, data.voldepol532_klett, data.pardepol532_klett, data.pardepolStd532_klett, data.voldepol532_raman, data.pardepol532_raman, data.pardepolStd532_raman, data.moldepol532, data.moldepolStd532, data.flagDefaultMoldepol532] = pollyxt_noa_depolratio(data, config);

% Angstroem exponent
[data.ang_ext_355_532_raman, data.ang_bsc_355_532_raman, data.ang_bsc_532_1064_raman, data.ang_bsc_355_532_klett, data.ang_bsc_532_1064_klett] = pollyxt_noa_angstrexp(data, config);
[data.ang_ext_355_532_raman_NR, data.ang_bsc_355_532_raman_NR, data.ang_bsc_355_532_klett_NR] = pollyxt_noa_NR_angstrexp(data, config);
fprintf('[%s] Finish.\n', tNow());

%% water vapor calibration
% get IWV from other instruments
fprintf('\n[%s] Start to water vapor calibration.\n', tNow());
[data.IWV, IWVAttri] = pollyxt_noa_read_IWV(data, config);
data.IWVAttri = IWVAttri;
[wvconst, wvconstStd, wvCaliInfo] = pollyxt_noa_wv_calibration(data, config);
% if not successful wv calibration, choose the default values
[data.wvconstUsed, data.wvconstUsedStd, data.wvconstUsedInfo] = pollyxt_noa_select_wvconst(wvconst, wvconstStd, data.IWVAttri, polly_parsetime(taskInfo.dataFilename, config.dataFileFormat), fullfile(processInfo.results_folder, campaignInfo.name, config.wvCaliFile), config.flagUsePreviousLC);
[data.wvmr, data.rh, ~, data.WVMR, data.RH] = pollyxt_noa_wv_retrieve(data, config, wvCaliInfo.IntRange);
fprintf('[%s] Finish.\n', tNow());

%% lidar calibration
fprintf('\n[%s] Start to lidar calibration.\n', tNow());
LC = pollyxt_noa_lidar_calibration(data, config);
data.LC = LC;
LCUsed = struct();
[LCUsed.LCUsed355, LCUsed.LCUsedTag355, LCUsed.flagLCWarning355, LCUsed.LCUsed532, LCUsed.LCUsedTag532, LCUsed.flagLCWarning532, LCUsed.LCUsed1064, LCUsed.LCUsedTag1064, LCUsed.flagLCWarning1064, LCUsed.LCUsed387, LCUsed.LCUsedTag387, LCUsed.flagLCWarning387, LCUsed.LCUsed607, LCUsed.LCUsedTag607, LCUsed.flagLCWarning607] = pollyxt_noa_mean_LC(data, config, taskInfo, fullfile(processInfo.results_folder, config.pollyVersion));
data.LCUsed = LCUsed;
fprintf('[%s] Finish.\n', tNow());

%% attenuated backscatter
fprintf('\n[%s] Start to calculate attenuated backscatter.\n', tNow());
[att_beta_355, att_beta_532, att_beta_1064, att_beta_387, att_beta_607] = pollyxt_noa_att_beta(data, config);
...
tc_mask = pollyxt_noa_targetclassi(data, config);
...
tc_mask_V2 = pollyxt_noa_targetclassi_V2(data, config);
...
if processInfo.flagEnableCaliResultsOutput
    ...
    pollyxt_noa_save_depolcaliconst(depCaliAttri.depol_cal_fac_532, depCaliAttri.depol_cal_fac_std_532, depCaliAttri.depol_cal_time_532, taskInfo.dataFilename, data.depol_cal_fac_532, data.depol_cal_fac_std_532, fullfile(processInfo.results_folder, campaignInfo.name, config.depolCaliFile532));
    pollyxt_noa_save_depolcaliconst(depCaliAttri.depol_cal_fac_355, depCaliAttri.depol_cal_fac_std_355, depCaliAttri.depol_cal_time_355, taskInfo.dataFilename, data.depol_cal_fac_355, data.depol_cal_fac_std_355, fullfile(processInfo.results_folder, campaignInfo.name, config.depolCaliFile355));

    %% save water vapor calibration results
    pollyxt_noa_save_wvconst(wvconst, wvconstStd, wvCaliInfo, data.IWVAttri, taskInfo.dataFilename, data.wvconstUsed, data.wvconstUsedStd, fullfile(processInfo.results_folder, campaignInfo.name, config.wvCaliFile));

    %% save lidar calibration results
    pollyxt_noa_save_LC_nc(data, taskInfo, config);
    pollyxt_noa_save_LC_txt(data, taskInfo, config);
...
if processInfo.flagEnableResultsOutput
    ...
    pollyxt_noa_save_overlap(data, taskInfo, config, overlapAttri, saveFile);

    %% save aerosol optical results
    pollyxt_noa_save_retrieving_results(data, taskInfo, config);
    pollyxt_noa_save_NR_retrieving_results(data, taskInfo, config);
    ...
    pollyxt_noa_save_voldepol(data, taskInfo, config);

    %% save quasi results
    pollyxt_noa_save_quasi_results(data, taskInfo, config);

    %% save quasi results V2
    pollyxt_noa_save_quasi_results_V2(data, taskInfo, config);

    %% save target classification results
    pollyxt_noa_save_tc(data, taskInfo, config);

    %% save target classification results V2
    pollyxt_noa_save_tc_V2(data, taskInfo, config);
...
if processInfo.flagEnableDataVisualization
    ...
    pollyxt_noa_display_quasiretrieving_V2(data, taskInfo, config);
    ...
    pollyxt_noa_display_longterm_cali(taskInfo, config);

    fprintf('[%s] Finish.\n', tNow());
end

%% get report
report = pollyxt_noa_results_report(data, taskInfo, config);
